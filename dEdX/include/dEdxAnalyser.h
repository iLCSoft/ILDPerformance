 ...
 *  The dEdxAnalyser processor loops through tracks (directly or via PFOs) to collect their dE/dx, fills various
 * histograms and calculates dE/dx resolution and separation power. Everything is stored in an AIDA-generated root-file.
 ...
 *  _nPart is the number of species, set in this header, while the PDG number and the names of the particles (for
 * labelling the histograms) are set in the init(). This version: _nPart = 5; species: electrons, muons, pions, kaon,
 * protons.
 *
 *  First, in processEvent(), the dE/dx value of each track is filled in the corresponding histogram, resulting in
 * Bethe-Bloch curves. The true PDG is received from the MCParticle linked to the track. In end(), these histograms are
 * then fitted with FitSlices, and the resulting mean and sigma values for each momentum bin are used to calculate
 * resolution and separation power. The separation power is calculated for each combination of the _nPart species. All
 * histograms, including the auto-generated fit results, are stored via AIDA in a root file. Finally, many of the
 * histograms can be plotted and saved as images to the current working directory.
 *
 *  Several special histograms for calibration purposes are provided.
 *  The FiducialElectrons histograms collects electrons which are similar to electrons at the test beam. The resulting
 * dE/dx resolution in ResNorm_FiducialElectrons should be used to calibrate the smearing factor in the
 * Compute_dEdxProcessor. The histogram NormLambdaFullAll_1 contains the mean dE/dx over |lambda| incl. a poly3 fit,
 * which is used in the AngularCorrection_dEdxProcessor for correction of the angular dependence. The histogram
 * NormCosThFullAll_1 contains the mean dE/dx over cos(theta), and shows the information of NormLambdaFullAll_1 over an
 * alternative angular scale. The Bethe-Bloch curves in BBHist are fitted to provide the parameters of the reference
 * curves for the dEdxPID in the LikelihoodPIDProcessor. The fit results are stored with the histograms and also printed
 * on the console at the end of the processor.
 *
 *  You can run this Analyser over any selection of tracks, e.g. events that contain only pion and kaon tracks.
 *  In that case, only the pion and kaon resolution as well as only the pion-kaon separation would give sensible plots,
 * but all other plots are be generated empty alongside. The Analyser has mostly been used to extract the dE/dx
 * performance from single-particle random-momentum files, but also to check e.g. 6f-ttbar events.
 *
 *  Note that the hit energy histograms will only be filled, if the track hits are available, which is usually only the
 * case for REC-files. The hit number histograms are unaffected by this.
 *
 *  @param _plotStuff - Set to true to switch on automatic image generation of many histograms in the current working
 ...
 *    Selection depends on root TPad::Print() capabilities, typically: [.ps, .eps, .pdf, .svg, .tex, .gif, .xpm, .png,
 * .jpg, .tiff, .cxx, .xml, .json, .root]. See
 * https://root.cern.ch/doc/master/classTPad.html#ad2fc5f449e4cb5480b9fb05fda3a8307 for details. string, default: .png
 *
 *  @param _usePFOTracks - Set to true to use tracks attached to charged PFO, instead of all tracks in the track
 * collection. This should be used with events which are more busy than single particle files. bool, default: false.
 *  @param _useOneTrack - Set true if from every event only the first object in the selected collection (track or PFO)
 * should be used. Usually, don't combine this with _usePFOTracks. bool, default: false.
 *  @param _usePFOTruthLink - Set true if the LCRelation from PFO to MCTruth instead of Track to MCTruth should be used
 * to determine MC PDG. Caution: the name of this relation still needs to be given in TrackMCTruthLink. bool, default:
 * false.
 *
 *  You can make a track selection via the optional parameters, e. g. to reduce 'contamination' from mis-linked Tracks
 * and MCParticles.
 *  @param _cutdEdx - Set true if particles with a very off dE/dx value (hard-coded) should be ignored.
 *    bool, default: false.
 *  @param _cutTrackPurity - Set true if particles should be ignored, where the MCParticle is connected to more than one
 ...
 *    double, default: 0.
 *  @param _cutMomMin - Tracks with a momentum smaller than the given value will be ignored. Set to 0 to accept all
 * particles. double, default: 0.
 *  @param _cutMomMax - Tracks with a momentum larger than the given value will be ignored. Set to 0 to accept all
 * particles. double, default: 0.
 *
 *  For calibration with beam test results, so-called fiducial electrons are selected, which ought to be similar to
 * beam-test conditions, and their properties stored in separate histograms.
 ...
 *    int, default: 200.
 *
 *  For some of the histograms, the axis binning can be adapted, which can be useful in case of very high or low
 * statistics.
...
protected:
  ...
  // pdg numbers and masses of particles, to be specified in ::init() (along with corresponding strings for names and
  // descriptions)
  ...
  // Storage for names of histograms to have a handle on fit results that are 'free floating' in memory, since
  // TH3::FitSlicesZ has no direct transfer
  ...
  TH2* _FitBoth_Res_1GeV[_nPart];  // resolution vs. lambda and vs. number of hits from fit results, for tracks with
                                   // p>1GeV
  ...
  // Vector array of all momentum, dE/dx, dE/dx error, tan lambda, number of hits and transverse momentum values for
  // each track
