...
void PlotHist(TObject* obj, string outpath) {
  ...
  if (obj->InheritsFrom("TH2")) {
    ...
    if (log)
      can1D->SetLogx();
    if (capmis) {
      hist->SetXTitle("dE/dx / (GeV/mm)");
      hist->SetYTitle("abundance");
    }
    if (cap_sp) {
      hist->SetYTitle("separation power");
    }
    can1D->Update();
...
void DirectoryHandler(TObject* dirobj, string outpath) {
  ...
  if (!dir->InheritsFrom("TFile"))
    newpath.append(dir->GetName());
