...
double BB_Fit(double* x, double* p) {
  ...
  return 1.35e-7 * (.5 * p[1] * TMath::Log(p[2] * pow(bg, 2) * pow(bg, 2)) - p[3] * bb * bb - p[4] * bg / 2.) /
         (bb * bb);
}
...
dEdxAnalyser::dEdxAnalyser() : Processor("dEdxAnalyser") {
  ...
  _description = "dEdxAnalyser: Looks into track dEdx values, calculates resolution and separation power, puts "
                 "everything in histograms and plots them.";

  registerInputCollection(LCIO::TRACK, "TrackCollection", "Name of input collection of reconstructed Tracks",
                          _trackColName, std::string("MarlinTrkTracks"));

  registerInputCollection(LCIO::LCRELATION, "TrackMCTruthLink",
                          "LCRelation from the reconstructed tracks to the MC particles", _tracktruthLinkName,
                          std::string("MarlinTrkTracksMCTruthLink"));

  registerInputCollection(LCIO::MCPARTICLE, "MCParticleCollection", "MC Particles", _MCParColName,
                          std::string("MCParticle"));

  registerInputCollection(LCIO::RECONSTRUCTEDPARTICLE, "PFOCollection", "Pandora PFOs", _PFOColName,
                          std::string("PandoraPFOs"));

  registerProcessorParameter("nBinsX", "Number of bins of the momentum axis in histograms. Default: 100.", _nBinsX,
                             int(100));

  registerProcessorParameter("nBinsY", "Number of bins of the dE/dx axis in histograms. Default: 300.", _nBinsY,
                             int(300));

  registerProcessorParameter(
      "minBinX", "log_10(momentum/GeV) of the lower end of the momentum axis in histograms. Default: -1 (= 0.1 GeV).",
      _minBinX, double(-1));

  registerProcessorParameter(
      "maxBinX", "log_10(momentum/GeV) of the upper end of the momentum axis in histograms. Default: 2 (= 100 GeV).",
      _maxBinX, double(2));

  registerProcessorParameter("minBinY", "Lower end of the dE/dx axis in GeV/mm in histograms. Default: 0 GeV/mm.",
                             _minBinY, double(0));

  registerProcessorParameter("maxBinY", "Upper end of the dE/dx axis in GeV/mm in histograms. Default: 1e-6 GeV/mm.",
                             _maxBinY, double(1e-6));

  registerProcessorParameter("usePFOTracks",
                             "Set true if tracks from (charged) PFOs should be used as data basis instead of all "
                             "tracks from the TrackCollection.",
                             _usePFOTracks, bool(false));

  registerProcessorParameter(
      "useOneTrack",
      "Set true if from every event only the first object in the selected collection (track or PFO) should be used.",
      _useOneTrack, bool(false));

  registerProcessorParameter(
      "usePFOTruthLink",
      "Set true if the LCRelation from PFO to MCTruth instead of Track to MCTruth should be used to determine MC PDG.",
      _usePFOTruthLink, bool(false));

  registerProcessorParameter("cutdEdx", "Set true if particles with a very off dE/dx value should be ignored.",
                             _cutdEdx, bool(false));

  registerProcessorParameter(
      "cutTrackPurity",
      "Set true if particles should be ignored, where the MCParticle is connected to more than one track.",
      _cutTrackPurity, bool(false));

  registerProcessorParameter("cutTrackPurityMom",
                             "Set true if particles should be ignored, where the MCParticle is connected to more than "
                             "one track, but the track with the highest momentum is still used.",
                             _cutTrackPurityMom, bool(false));
  ...
  registerProcessorParameter("cutLamMin",
                             "Tracks with an angle lambda (relative to the cathode) smaller than the given value will "
                             "be ignored. Set to 0 to accept all particles.",
                             _cutLamMin, double(0));

  registerProcessorParameter("cutLamMax",
                             "Tracks with an angle lambda (relative to the cathode) larger than the given value will "
                             "be ignored. Set to 0 to accept all particles.",
                             _cutLamMax, double(0));

  registerProcessorParameter("fiducialMomMin", "Fiducial minimum absolut momentum / GeV; default: 3.", _fidMomMin,
                             double(3));

  registerProcessorParameter("fiducialMomMax", "Fiducial maximum absolut momentum / GeV; default: 10.", _fidMomMax,
                             double(10));

  registerProcessorParameter("fiducialLamMin", "Fiducial minimum absolut lambda / deg; default: 1.", _fidLamMin,
                             double(1));

  registerProcessorParameter("fiducialLamMax", "Fiducial maximum absolut lambda / deg; default: 10.", _fidLamMax,
                             double(10));

  registerProcessorParameter("fiducialNHitsMin", "Fiducial minimum number of hits; default: 200.", _fidNHitsMin,
                             int(200));

  registerProcessorParameter("LikelihoodPIDMethod",
                             "Name of PID algorithm in LikelihoodPIDProcessor to be used in PFO vs. truth comparison "
                             "matrix; default: 'dEdxPID'.",
                             _LikelihoodPIDMethod, std::string("dEdxPID"));

  registerProcessorParameter(
      "plotStuff",
      "Set true to automatically plot various histograms as .png in the current working directory. Default: false.",
      _plotStuff, bool(false));

  registerProcessorParameter("plotFolder",
                             "Specify the folder in which the automatically plotted images should be stored (if "
                             "chosen). Needs to exist. Default: '.' [current working directory].",
                             _plotFolder, std::string("."));

  registerProcessorParameter("useLCTPCStyle",
                             "Set true if you would like to use the LCTPCStyle.C from MarlinTPC/analysis/rootscripts "
                             "as style for the plots. Default: false.",
                             _useLCTPCStyle, bool(false));

  registerProcessorParameter("fileFormat",
                             "Specify the format of the automatically plotted images (if chosen). Selection depends on "
                             "root TPad::Print() capabilities, typically: [.ps, .eps, .pdf, .svg, .tex, .gif, .xpm, "
                             ".png, .jpg, .tiff, .cxx, .xml, .json, .root]. Default: .png.",
...
void dEdxAnalyser::init() {
  ...
  for (int i = 0; i < _nBinsX + 1; i++)
    histbinsX[i] = pow(10, _minBinX + (_maxBinX - _minBinX) * i / (float)(_nBinsX));
  for (int i = 0; i < _nBinsX + 1; i++)
    histbinsX2[i] = pow(10, minBinX2 + (_maxBinX - minBinX2) * i / (float)(_nBinsX));
  for (int i = 0; i < _nBinsX + 1; i++)
    histbinsX3[i] = pow(10, _minBinX + (maxBinX2 - _minBinX) * i / (float)(_nBinsX));
  for (int i = 0; i < _nBinsY + 1; i++)
    histbinsY[i] = _minBinY + (_maxBinY - _minBinY) * i / (float)(_nBinsY);
  for (int i = 0; i < _nBinsY + 1; i++)
    histbinsY2[i] = _minBinY + (maxBinY2 - _minBinY) * i / (float)(_nBinsY);
  std::vector<std::string> Particles = {"Electrons", "Muons", "Pions", "Kaons",
                                        "Protons",   "Other", "All"}; // size: _nPart+2
  std::vector<std::string> particles = {"electrons",           "muons",        "pions", "kaons", "protons",
                                        "all other particles", "all particles"}; // size: _nPart+2
  ...
  for (int i = 0; i < _nPart + 2; i++) {
    ...
    s1 << "BB" << Particles[i];
    std::stringstream s2;
    s2 << "Bethe-Bloch histogram for " << particles[i];
  ...
  for (int i = 0; i < _nPart; i++)
    _FitHist[i] = new TObjArray();
  for (int i = 0; i < _nPart; i++)
    _FitLambda[i] = new TObjArray();
  for (int i = 0; i < _nPart + 1; i++)
    _FitNHit[i] = new TObjArray();
  for (int i = 0; i < _nPart; i++)
    _FitBoth[i] = new TObjArray();
  for (int i = 0; i < _nPart; i++)
    _FitBoth_1GeV[i] = new TObjArray();
  for (int i = 0; i < _nPart + 1; i++)
    _FitLambdaFull[i] = new TObjArray();
  for (int i = 0; i < _nPart + 1; i++)
    _FitCosThFull[i] = new TObjArray();

  // create resolution and separation power histograms
  int sp_c = 0;
  for (int i = 0; i < _nPart; i++) {
    std::stringstream s1;
    s1 << "BB" << Particles[i] << "_FitRes";
    std::stringstream s2;
    s2 << "dE/dx resolution over momentum for " << particles[i] << " from fit result";
    ...
    std::stringstream s3;
    s3 << "BB" << Particles[i] << "_FitResSum";
    std::stringstream s4;
    s4 << "dE/dx resolution distribution above 1 GeV for " << particles[i] << " from fit result";
    ...
    std::stringstream s5;
    s5 << "Res" << Particles[i] << "_Norm";
    std::stringstream s6;
    s6 << "Relative dE/dx resolution distribution for " << particles[i] << " from all entries above .3 GeV";
    ...
    std::stringstream s7;
    s7 << "Res" << Particles[i] << "_Norm_1GeV";
    std::stringstream s8;
    s8 << "Relative dE/dx resolution distribution for " << particles[i] << " from all entries above 1 GeV";
    ...
    std::stringstream s9;
    s9 << "NormLambda" << Particles[i] << "";
    std::stringstream s10;
    s10 << "Relative dE/dx distribution over lambda with p>1 GeV and nHit>=200 for " << particles[i]
        << " from all entries";
    ...
    s11 << "NormLambda" << Particles[i] << "_FitRes";
    std::stringstream s12;
    s12 << "dE/dx resolution for " << particles[i] << " over lambda from fit result";
    ...
    std::stringstream s13;
    s13 << "NormNHit" << Particles[i] << "";
    std::stringstream s14;
    s14 << "Relative dE/dx distribution over number of hits with p>1GeV and |lambda|<20deg for " << particles[i]
        << " from all entries";
    ...
    std::stringstream s15;
    s15 << "NormNHit" << Particles[i] << "_FitRes";
    std::stringstream s16;
    s16 << "dE/dx resolution for " << particles[i] << " over number of hits from fit result";
    ...
    std::stringstream s20;
    s20 << "NormBoth" << Particles[i] << "";
    std::stringstream s21;
    s21 << "Relative dE/dx distribution over lambda and number of hits for " << particles[i] << " from all entries";
    ...
    s22 << "NormBoth" << Particles[i] << "_FitRes";
    std::stringstream s23;
    s23 << "dE/dx resolution for " << particles[i] << " over lambda and number of hits from fit result";
    ...
    std::stringstream s24;
    s24 << "NormBoth" << Particles[i] << "_1GeV";
    std::stringstream s25;
    s25 << "Relative dE/dx distribution over lambda and number of hits with p>1GeV for " << particles[i]
        << " from all entries";
    ...
    std::stringstream s26;
    s26 << "NormBoth" << Particles[i] << "_FitRes_1GeV";
    std::stringstream s27;
    s27 << "dE/dx resolution for " << particles[i] << " with p>1GeV over lambda and number of hits from fit result";
    ...
    std::stringstream s28;
    s28 << "NormLambdaFull" << Particles[i] << "";
    std::stringstream s29;
    s29 << "Relative dE/dx distribution over lambda with p>1 GeV for " << particles[i] << " from all entries";
    ...
    std::stringstream s30;
    s30 << "NormLambdaFull" << Particles[i] << "_FitRes";
    std::stringstream s31;
    s31 << "dE/dx resolution for " << particles[i] << " over lambda from fit result";
    ...
    s32 << "NormCosThFull" << Particles[i] << "";
    std::stringstream s33;
    s33 << "Relative dE/dx distribution over cos(theta) with p>1 GeV for " << particles[i] << " from all entries";
    ...
    std::stringstream s34;
    s34 << "NormCosThFull" << Particles[i] << "_FitRes";
    std::stringstream s35;
    s35 << "dE/dx resolution for " << particles[i] << " over cos(theta) from fit result";
    ...
    {
      std::stringstream s17;
      s17 << "SP_" << SP_part[i] << "_" << SP_part[j];
      std::stringstream s18;
      s18 << "Separation power between " << particles[i] << " and " << particles[j];
  ...
  std::stringstream s13;
  s13 << "NormNHit" << Particles[_nPart + 1] << "";
  std::stringstream s14;
  s14 << "Relative dE/dx distribution over number of hits with p>1GeV and |lambda|<20deg for " << particles[_nPart + 1]
      << " from all entries";
  ...
  std::stringstream s15;
  s15 << "NormNHit" << Particles[_nPart + 1] << "_FitRes";
  std::stringstream s16;
  s16 << "dE/dx resolution for " << particles[_nPart + 1] << " over number of hits from fit result";
  ...
  std::stringstream s38;
  s38 << "NormLambdaFull" << Particles[_nPart + 1] << "";
  std::stringstream s39;
  s39 << "Relative dE/dx distribution over lambda with p>1 GeV for " << particles[_nPart + 1] << " from all entries";
  ...
  std::stringstream s40;
  s40 << "NormLambdaFull" << Particles[_nPart + 1] << "_FitRes";
  std::stringstream s41;
  s41 << "dE/dx resolution for " << particles[_nPart + 1] << " over lambda from fit result";
  ...
  std::stringstream s42;
  s42 << "NormCosThFull" << Particles[_nPart + 1] << "";
  std::stringstream s43;
  s43 << "Relative dE/dx distribution over cos(theta) with p>1 GeV for " << particles[_nPart + 1]
      << " from all entries";
  ...
  std::stringstream s44;
  s44 << "NormCosThFull" << Particles[_nPart + 1] << "_FitRes";
  std::stringstream s45;
  s45 << "dE/dx resolution for " << particles[_nPart + 1] << " over cos(theta) from fit result";
  ...
  // fiducial electrons
  _ResNorm_FiducialElectrons =
      new TH1D("ResNorm_FiducialElectrons", "Relative dE/dx resolution distribution for fiducial electrons", 100, 0, 2);

  // some other histograms for cross-checks etc.
  _ResElectrons = new TH1D("ResElectrons",
                           "dE/dx resolution distribution above .3 GeV for electrons from all entries, not normalised",
                           100, _minBinY, _maxBinY / 2.);
  _MCmult = new TH1D("MCmult", "MC particle multiplicity of linked reco objects (tracks or PFOs)", 11, -.5, 10.5);

  _PDGCheck = new TH2I("PDGCheck", "PDG cross check PFO vs. MCTruth", _nPart + 1, -.5, _nPart + .5, _nPart + 1, -.5,
                       _nPart + .5);
  _PDGCheck_weight = new TH2D("PDGCheck_weight", "PDG cross check PFO vs. MCTruth, likelihood weighted", _nPart + 1,
                              -.5, _nPart + .5, _nPart + 1, -.5, _nPart + .5);
  _PDGCheck_eff = new TH2D("PDGCheck_eff", "PDG cross check PFO vs. MCTruth, efficiency (vertical sum = 1)", _nPart + 1,
                           -.5, _nPart + .5, _nPart + 1, -.5, _nPart + .5);
  _PDGCheck_pur = new TH2D("PDGCheck_pur", "PDG cross check PFO vs. MCTruth, purity (horizontal sum = 1)  ", _nPart + 1,
                           -.5, _nPart + .5, _nPart + 1, -.5, _nPart + .5);
  ...
  _NTracksMomHist = new TH1D("NTracksMomHist",
                             "Number of tracks of each event actually used after cuts and with p > 100 MeV", 20, 0, 0);
  ...
  _TrackSiblingsMomHist =
      new TH1D("TrackSiblingsMomHist", "Momentum of tracks after cuts in events with nTracks > 1", _nBinsX, histbinsX2);
  _TrackSiblingsMomHist->SetXTitle("momentum (GeV)");
  _TrackSiblingsMomHist->SetYTitle("abundance");

  _dEdxOutlierAbundance =
      new TH1D("dEdxOutlierAbundance", "Number of tracks with dE/dx values that don't match the MC PDG", 5, -.5, 4.5);
  ...
  _dEdxOutlierProportion = new TH1D("dEdxOutlierProportion",
                                    "Proportion of tracks with dE/dx values that don't match the MC PDG", 5, -.5, 4.5);
  ...
  _dEdxErrorCorrelation = new TH2D("dEdxErrorCorrelation", "Correlation plot of dE/dx error vs. dE/dx as cross check",
                                   100, _minBinY, _maxBinY, 100, _minBinY, _maxBinY * .1);

  // hit number and energy spectra

  for (int i = 0; i < _nPart + 2; i++) {
    std::stringstream s3;
    s3 << "HitNum_" << Particles[i];
    std::stringstream s4;
    s4 << "Hit number spectrum for " << particles[i];
  ...
  for (int i = 0; i < _nPart + 2; i++) {
    std::stringstream s1;
    s1 << "HitEng_" << Particles[i];
    std::stringstream s2;
    s2 << "Hit energy spectrum for " << particles[i];
    ...
    std::stringstream s5;
    s5 << "HitEng_at1_" << Particles[i];
    std::stringstream s6;
    s6 << "Hit energy spectrum around 1 GeV for " << particles[i];
    ...
    std::stringstream s7;
    s7 << "HitEng_at25_" << Particles[i];
    std::stringstream s8;
    s8 << "Hit energy spectrum around 2.5 GeV for " << particles[i];
    ...
    std::stringstream s9;
    s9 << "HEHist_" << Particles[i];
    std::stringstream s10;
    s10 << "Hit energy curves over momentum for " << particles[i];
...
void dEdxAnalyser::processEvent(LCEvent* evt) {
  ...
  for (int i = 0; i < n_track_mod; ++i) {
    ...
    if (_usePFOTracks) {
      ...
      track = dynamic_cast<Track*>(col_track->getElementAt(i));
    ...
    const MCParticleVec& vec_mcpar = _usePFOTruthLink ? (MCParticleVec&)nav_t2trel.getRelatedToObjects(pfo)
                                                      : (MCParticleVec&)nav_t2trel.getRelatedToObjects(track);
    const FloatVec& wei_mcpar =
        _usePFOTruthLink ? nav_t2trel.getRelatedToWeights(pfo) : nav_t2trel.getRelatedToWeights(track);
    ...
    if (vec_mcpar.size() != 0) {
      for (unsigned int j = 0; j < std::min(vec_mcpar.size(), wei_mcpar.size()); j++) {
        ...
        if (trackwgt > .5 && trackwgt > maxweight) {
          ...
          if (mcpdg == 11 || mcpdg == 13 || mcpdg == 211 || mcpdg == 321 || mcpdg == 2212)
            pdg = mcpdg;
          else
            pdg = 10; // flag: other particle
          maxweight = trackwgt;
        }
      }
    }
    ...
    if (pdg != 0 && dEdx > 1e-8 && mcpar) // make sure, there is a mcpar linked and a finite dE/dx
    {
      ...
      trkHelixC.Initialize_Canonical(track->getPhi(), track->getD0(), track->getZ0(), track->getOmega(),
                                     track->getTanLambda(), _bField);
      double mom = sqrt(pow(trkHelixC.getMomentum()[0], 2) + pow(trkHelixC.getMomentum()[1], 2) +
      ...
      if (_cutLamMin)
        if (lambda < _cutLamMin)
          continue;
      if (_cutLamMax)
        if (lambda > _cutLamMax)
          continue;
      _CutAnaHist->Fill(icut);
      ++icut;

      // If more other tracks are connected to the same MCParticle and this is not the track with the highest momentum
      // among them, skip this track.
      if (_cutTrackPurityMom)
        if (n_rel_tracks > 1) {
          ...
          for (int j = 0; j < n_rel_tracks; ++j) {
            ...
            tempHelix.Initialize_Canonical(tempTrack->getPhi(), tempTrack->getD0(), tempTrack->getZ0(),
                                           tempTrack->getOmega(), tempTrack->getTanLambda(), _bField);
            double tempmom = sqrt(pow(tempHelix.getMomentum()[0], 2) + pow(tempHelix.getMomentum()[1], 2) +
                                  pow(tempHelix.getMomentum()[2], 2));
            if (tempmom > maxtrackmom)
              maxtrackmom = tempmom;
          }
          if (mom < maxtrackmom)
            continue;
      ...
      if (_cutD0)
        if (fabs(track->getD0()) > _cutD0)
          continue;
      if (_cutZ0)
        if (fabs(track->getZ0()) > _cutZ0)
          continue;
      _CutAnaHist->Fill(icut);
      ++icut;
      ...
      if (pdg == 211 && dEdx > (pow(log10(mom), 2) + .2) * 1e-6) {
        streamlog_out(DEBUG) << "Event " << evt->getEventNumber()
                             << ": dE/dx does not match!  PDG = 211, dE/dx = " << dEdx
                             << ", upper limit = " << (pow(log10(mom), 2) + .2) * 1e-6 << std::endl;
        _dEdxOutlierAbundance->Fill(0);
        cutdEdx = true;
      }
      if (pdg == 321 && dEdx > (pow(log10(mom) * 1.3, 2) + .2) * 1e-6) {
        streamlog_out(DEBUG) << "Event " << evt->getEventNumber()
                             << ": dE/dx does not match!  PDG = 321, dE/dx = " << dEdx
                             << ", upper limit = " << (pow(log10(mom) * 1.3, 2) + .2) * 1e-6 << std::endl;
      ...
      if (pdg == 321 && dEdx < (.28 - mom / 3.) * 1e-6) {
        streamlog_out(DEBUG) << "Event " << evt->getEventNumber()
                             << ": dE/dx does not match!  PDG = 321, dE/dx = " << dEdx
                             << ", lower limit = " << (.28 - mom / 3.) * 1e-6 << std::endl;
        _dEdxOutlierAbundance->Fill(2);
        cutdEdx = true;
      }
      if (pdg == 2212 && dEdx < (.3 - mom / 5.) * 1e-6) {
        streamlog_out(DEBUG) << "Event " << evt->getEventNumber()
                             << ": dE/dx does not match!  PDG = 2212, dE/dx = " << dEdx
                             << ", lower limit = " << (.3 - mom / 5.) * 1e-6 << std::endl;
      ...
      if (n_track > 1)
        _TrackSiblingsMomHist->Fill(mom);
      ...
      for (int j = 0; j < _nPart; j++) {
        if (pdg == _PDG[j]) {
          ...
          if (mom > .3 && mom < 100)
            _dEdxVec[j].emplace_back(
                std::array<double, 6>{{mom, dEdx, track->getdEdxError(), track->getTanLambda(), (double)nHits, mom_t}});
        }
      }
      if (pdg == 10)
        _BBHist[_nPart]->Fill(mom, dEdx);
      _BBHist[_nPart + 1]->Fill(mom, dEdx);

      if (pdg == 11 && mom > .3)
        _ResElectrons->Fill(dEdx);
        ...
        for (int j = 0; j < _nPart + 1; j++) {
          ...
          for (int k = 0; k < _nPart + 1; k++) {
            if (PFO_pdg != _PDG[k])
              continue;
            _PDGCheck->Fill(j, k);
            _PDGCheck_weight->Fill(
                j, k, pidh->getParticleID(pfo, pidh->getAlgorithmID(_LikelihoodPIDMethod)).getLikelihood());
          }
        }
      }
      ...
      for (unsigned int h = 0; h < trkHits.size(); ++h) {
        ...
        for (int j = 0; j < _nPart; j++)
          if (pdg == _PDG[j])
            _HitEnergySpectrum[j]->Fill(EDep);
        if (pdg == 10)
          _HitEnergySpectrum[_nPart]->Fill(EDep);
        _HitEnergySpectrum[_nPart + 1]->Fill(EDep);
        if (.9 < mom && mom < 1.15) // around 1 GeV
        {
          for (int j = 0; j < _nPart; j++)
            if (pdg == _PDG[j])
              _HitEnergySpectrum_at1[j]->Fill(EDep);
          if (pdg == 10)
            _HitEnergySpectrum_at1[_nPart]->Fill(EDep);
        ...
        if (2 < mom && mom < 3) // around 2.5 GeV
        {
          for (int j = 0; j < _nPart; j++)
            if (pdg == _PDG[j])
              _HitEnergySpectrum_at25[j]->Fill(EDep);
          if (pdg == 10)
            _HitEnergySpectrum_at25[_nPart]->Fill(EDep);
        ...
        if (1) //(mom_t>=1 && nHits>=200)
        {
          for (int j = 0; j < _nPart; j++)
            if (pdg == _PDG[j])
              _HEHist[j]->Fill(mom, EDep);
          if (pdg == 10)
            _HEHist[_nPart]->Fill(mom, EDep);
      ...
      for (int j = 0; j < _nPart; j++)
        if (pdg == _PDG[j])
          _HitNumberSpectrum[j]->Fill(nHits);
      if (pdg == 10)
        _HitNumberSpectrum[_nPart]->Fill(nHits);
...
void dEdxAnalyser::end() {
  ...
  for (int i = 0; i < _nPart; i++)
    _BBHist[i]->FitSlicesY(gausfit, 0, -1, 0, "QR", _FitHist[i]);
  ...
  for (int i = 0; i < _nPart; i++) {
    ...
    for (unsigned int v = 0; v < _dEdxVec[i].size(); v++) // Normalised Resolution
    {
      ...
      if (binMean > 0) {
        ...
        if (mom_t > 1)
          _ResNormHist_1GeV[i]->Fill(normMean);

        if (mom_t > 1 && nHits >= 200)
          _NormLambdaHist[i]->Fill(lambda, normMean);
        if (lambda < 20)
          _NormNHitHist[i]->Fill(nHits, normMean);
        if (1)
          _NormNHitHist[_nPart]->Fill(nHits, normMean);

        if (1)
          _NormBothHist[i]->Fill(lambda, nHits, normMean);
        if (mom_t > 1)
          _NormBothHist_1GeV[i]->Fill(lambda, nHits, normMean);

        if (mom_t > 1)
          _NormLambdaFullHist[i]->Fill(lambda, normMean);
        if (mom_t > 1)
          _NormLambdaFullHist[_nPart]->Fill(lambda, normMean);

        if (mom_t > 1)
          _NormCosThFullHist[i]->Fill(costh, normMean);
        if (mom_t > 1)
          _NormCosThFullHist[_nPart]->Fill(costh, normMean);

        // fiducial electrons
        if (i == 0 && _fidMomMin <= mom && mom <= _fidMomMax && _fidLamMin <= lambda && lambda <= _fidLamMax &&
            nHits >= _fidNHitsMin)
          _ResNorm_FiducialElectrons->Fill(normMean);

        if (i == 1 && normMean > 1.5) {
          streamlog_out(MESSAGE) << v << " " << _dEdxVec[i][v][0] << " " << _dEdxVec[i][v][1] << " "
                                 << _dEdxVec[i][v][2] << " " << _dEdxVec[i][v][3] << " " << _dEdxVec[i][v][4] << " "
                                 << _dEdxVec[i][v][5] << " " << normMean << " " << relError << std::endl;
        }
      }
    }
    ...
    for (int d = 1; d <= 18; d++) // nBinsX lambda
    {
      for (int e = 1; e <= 110; e++) // nBinsX nHits up to 220
      {
        ...
        if (std::isnan(res) || std::isnan(reserr) || ProjZ->GetBinContent(d, e) == 0) {
          res = 0;
          reserr = 0;
        }
    ...
    _NormBothHist_1GeV[i]->FitSlicesZ(gausfitrel, 0, -1, 0, -1, 0,
                                      "QR"); // angular and number-of-hits distribution above 1 GeV
    ...
    for (int d = 1; d <= 18; d++) // nBinsX lambda
    {
      for (int e = 1; e <= 110; e++) // nBinsX nHits up to 220
      {
        ...
        double res = 1. * si / mu;
        double reserr = sqrt(pow(1. * dsi / mu, 2) + pow(1. * si / mu / mu * dmu, 2));
        if (std::isnan(res) || std::isnan(reserr) || ProjZ_1GeV->GetBinContent(d, e) == 0) {
          res = 0;
          reserr = 0;
        }
    ...
    _NormLambdaFullHist[i]->FitSlicesY(gausfitrel, 0, -1, 0, "QR",
                                       _FitLambdaFull[i]); // angular distribution in lambda, no nHits cut
    ...
    _NormCosThFullHist[i]->FitSlicesY(gausfitrel, 0, -1, 0, "QR",
                                      _FitCosThFull[i]); // angular distribution in cos(theta), no nHits cut
    ...
    for (int j = i + 1; j < _nPart; j++) // Separation Power
    {
      ...
      for (int n = 1; n < _nBinsX + 1; n++) // 0 is underflow bin, nBinsx+1 overflow bin
      {
        ...
        streamlog_out(DEBUG) << mu1 << " " << mu1e << " " << mu2 << " " << mu2e << " " << s1 << " " << s1e << " " << s2
                             << " " << s2e << std::endl;
        ...
        streamlog_out(DEBUG) << "  " << sp << " " << sqrt(spe_1) << " " << sqrt(spe_2) << " " << sqrt(spe_3) << " "
                             << sqrt(spe_4) << " " << spe << std::endl;
  ...
  _NormLambdaFullHist[_nPart]->FitSlicesY(gausfitrel, 0, -1, 0, "QR",
                                          _FitLambdaFull[_nPart]); // angular distribution in lambda, no nHits cut
  ...
  _NormCosThFullHist[_nPart]->FitSlicesY(gausfitrel, 0, -1, 0, "QR",
                                         _FitCosThFull[_nPart]); // angular distribution in cos(theta), no nHits cut
  ...
  for (int i = 0; i < _nPart; i++)
    _HEHist[i]->FitSlicesY(landaufit, 0, -1, 0, "QR", _HEFitHist[i]);
  ...
  if (_plotStuff) {
    ...
    std::vector<double> YMax = {1000, _maxBinY, _maxBinY / 10.,
                                10};               // y-axis maxima of dE/dx fit result histogram plots
    std::vector<double> YMaxn = {1000, 2, .2, 10}; // y-axis maxima of normalised fit result histogram plots
    std::vector<double> YMaxHE = {1e6, _maxBinY * 6, _maxBinY * 6 / 10.,
                                  100}; // y-axis maxima of hit energy fit result histogram plots
    ...
    for (int i = 0; i < _nPart; i++) {
      ...
      if (ptr_BBfit[i].Get())
        ...
        streamlog_out(MESSAGE) << "Mass " << _Masses[i] << ": fit result pointer empty!" << std::endl;
      }
    }
    ...
    if (ptr_nhit.Get())
      ...
      streamlog_out(MESSAGE) << FitNHit_mean->GetName() << ": fit result pointer empty!" << std::endl;
    }
    ...
    if (ptr_lm_pol2.Get())
      ...
      streamlog_out(MESSAGE) << FitLambdaFull_mean->GetName() << ": poly2 fit result pointer empty!" << std::endl;
    }
    if (ptr_lm_pol3.Get())
      ptr_lm_pol3->Print();
    else {
      streamlog_out(MESSAGE) << FitLambdaFull_mean->GetName() << ": poly3 fit result pointer empty!" << std::endl;
    }
    ...
    if (ptr_ac.Get())
      ...
      streamlog_out(MESSAGE) << FitCosThFull_mean->GetName() << ": fit result pointer empty!" << std::endl;
    }
  ...
  if (ptr_fe.Get()) {
    streamlog_out(MESSAGE) << "\nFiducial electrons dE/dx resolution: "
                           << ptr_fe->Parameter(2) / ptr_fe->Parameter(1) * 100 << " %" << std::endl;
  ...
  if (ptr_lm_pol3.Get()) {
    ...
    streamlog_out(MESSAGE) << ptr_lm_pol3->Parameter(0) << " " << ptr_lm_pol3->Parameter(1) << " "
                           << ptr_lm_pol3->Parameter(2) << " " << ptr_lm_pol3->Parameter(3) << "\nwith a chi2/ndf of  "
                           << ptr_lm_pol3->Chi2() << "/" << ptr_lm_pol3->Ndf() << std::endl;
    outfile << "# Angular dependence fit (poly3 vs. lambda) result:" << std::endl;
    outfile << ptr_lm_pol3->Parameter(0) << " " << ptr_lm_pol3->Parameter(1) << " " << ptr_lm_pol3->Parameter(2) << " "
  ...
  for (int i = 0; i < _nPart; ++i) {
    if (ptr_BBfit[i].Get()) {
      streamlog_out(MESSAGE) << _Masses[i] << "  " << ptr_BBfit[i]->Chi2() << "/" << ptr_BBfit[i]->Ndf() << "  "
                             << ptr_BBfit[i]->Parameter(1) << " " << ptr_BBfit[i]->Parameter(2) << " 1 "
                             << ptr_BBfit[i]->Parameter(3) << " " << ptr_BBfit[i]->Parameter(4) << " " << std::endl;
      outfile << _Masses[i] << "  " << ptr_BBfit[i]->Chi2() << "/" << ptr_BBfit[i]->Ndf() << "  "
              << ptr_BBfit[i]->Parameter(1) << " " << ptr_BBfit[i]->Parameter(2) << " 1 " << ptr_BBfit[i]->Parameter(3)
  ...
  for (unsigned int j = 0; j < 10; j++)
    streamlog_out(DEBUG) << j << "  " << _CutAnaHist->GetBinContent(j) << std::endl;

  streamlog_out(DEBUG) << "NTracksHist Entries" << std::endl;
  for (unsigned int j = 0; j < 10; j++)
    streamlog_out(DEBUG) << j << "  " << _NTracksHist->GetBinContent(j) << std::endl;

  streamlog_out(DEBUG) << "NTracksMomHist Entries" << std::endl;
  for (unsigned int j = 0; j < 10; j++)
    streamlog_out(DEBUG) << j << "  " << _NTracksMomHist->GetBinContent(j) << std::endl;

  streamlog_out(DEBUG) << "TrackMomHist Entries" << std::endl;
  for (int j = 0; j < _nBinsX; j++)
    streamlog_out(DEBUG) << j << "  " << histbinsX2[j] << "  " << _TrackMomHist->GetBinContent(j) << std::endl;

  // streamlog_out(DEBUG) << "MCmomVec Entries" << std::endl;
  // for (unsigned int j=0; j<_MCmomVec.size()/3; j++) streamlog_out(DEBUG) << j << "  " << _MCmomVec[3*j] << "  " <<
  // _MCmomVec[3*j+1] << "  " << _MCmomVec[3*j+2] << std::endl;
...
void dEdxAnalyser::PlotTH1(TCanvas* can, TH1* hist) {
  ...
  can->Update();
  std::stringstream s;
  s << _plotFolder << "/" << hist->GetName() << _fileFormat;
...
void dEdxAnalyser::PlotTH1_FPn(TCanvas* can, TH1* hist, int n) {
  ...
  can->Update();
  std::stringstream s;
  s << _plotFolder << "/" << hist->GetName() << "_FitPar" << n << _fileFormat;
  can->Print(s.str().c_str());
}
