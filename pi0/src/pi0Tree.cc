...
Pi0Tree::Pi0Tree() : Processor("Pi0Tree") {
  ...
  registerInputCollection(LCIO::RECONSTRUCTEDPARTICLE, "GammaGammaParticleCollection",
                          "Name of the gamma-gamma candidate input collection", _gammaGammaParticleCollectionName,
...
void Pi0Tree::processEvent(LCEvent* evt) {
  ...
  while (MCParticle* mcp = mcpIt.next()) {
    ...
    if (keep) {
      ...
      streamlog_out(DEBUG) << " Found GammaGamma decaying parent MCParticle " << "Index = " << index_mcp
                           << " pdgID = " << mcp->getPDG() << ", genstat = " << mcp->getGeneratorStatus()
                           << ", E = " << mcp->getEnergy() << std::endl;
      streamlog_out(DEBUG) << " Momentum = " << mcp->getMomentum()[0] << " " << mcp->getMomentum()[1] << " "
                           << mcp->getMomentum()[2] << std::endl;
      streamlog_out(DEBUG) << " Vertex =   " << mcp->getVertex()[0] << " " << mcp->getVertex()[1] << " "
                           << mcp->getVertex()[2] << std::endl;
      streamlog_out(DEBUG) << " Endpoint = " << mcp->getEndpoint()[0] << " " << mcp->getEndpoint()[1] << " "
      ...
      for (unsigned int idaughter = 0; idaughter < daughters.size(); idaughter++) {
        ...
        streamlog_out(DEBUG) << " Daughter MCParticle " << idaughter << " pdg ID = " << mcd->getPDG()
                             << ", genstat = " << mcd->getGeneratorStatus() << ", E = " << mcd->getEnergy()
                             << std::endl;
        streamlog_out(DEBUG) << " Momentum = " << mcd->getMomentum()[0] << " " << mcd->getMomentum()[1] << " "
                             << mcd->getMomentum()[2] << std::endl;
        streamlog_out(DEBUG) << " Vertex =   " << mcd->getVertex()[0] << " " << mcd->getVertex()[1] << " "
                             << mcd->getVertex()[2] << std::endl;
        streamlog_out(DEBUG) << " Endpoint = " << mcd->getEndpoint()[0] << " " << mcd->getEndpoint()[1] << " "
                             << mcd->getEndpoint()[2] << std::endl;
        ...
        if (maxcaloweight > 0.1) {
          streamlog_out(MESSAGE) << " found reco photon for mcd at imaxcaloweight = " << imaxcaloweight
  ...
  streamlog_out(DEBUG) << " nMCGammaGammaParticles Found = " << nMCPi0 << " ( " << nMCPi0 - nMCEta - nMCEtaPrime << " "
                       << nMCEta << " " << nMCEtaPrime << " )" << " Esum = " << ESum_MCP << std::endl;
  ...
  streamlog_out(DEBUG) << " got rec2mcNav from " << rec2mcNav.getFromType() << " to " << rec2mcNav.getToType()
                       << std::endl;
  ...
  while (ReconstructedParticle* ggp = ggpIt.next()) {
    ...
    streamlog_out(DEBUG) << " GammaGammaParticle " << nRecoPi0 << " type = " << ggp->getType()
                         << " E = " << ggp->getEnergy() << " GoodnessOfPid " << ggp->getGoodnessOfPID() << std::endl;
    streamlog_out(DEBUG) << " Momentum = " << ggp->getMomentum()[0] << " " << ggp->getMomentum()[1] << " "
    ...
    for (unsigned int igamma = 0; igamma < gammas.size(); igamma++) {
      ...
      ESum_Reco_Meas +=
          gammas[igamma]->getEnergy(); // Measured Energy Sum of the Photons that are fitted to GammaGammaParticles
      for (int i = 0; i < 3; i++)
        sumP[i] += gammas[igamma]->getMomentum()[i];
      ...
      if (!isConversion) {
        ...
        streamlog_out(DEBUG) << " found true photon at imaxcaloweight = " << imaxcaloweight
                             << " with weight = " << maxcaloweight << std::endl;
        streamlog_out(DEBUG) << " found true photon at imaxtrckweight = " << imaxtrckweight
        ...
        if (maxweight < 0.1) {
          streamlog_out(DEBUG) << " weight of photon = " << maxweight << " is too small - don't count as correct"
                               << std::endl;
          continue;
        }

        streamlog_out(DEBUG) << " found true photon at imaxweight = " << imaxweight << " with weight = " << maxweight
                             << std::endl;
    ...
    if (mcgps[0] && mcgps[1] && mcgps[0] == mcgps[1]) {
      ...
      if (istrue) {
        pdg = mcgps[0]->getPDG();
        streamlog_out(MESSAGE) << "(fully correct) PDG of both photons parent is = " << pdg
        ...
        streamlog_out(MESSAGE) << "(Non-prompt associated) PDG of both photons parent is = " << pdg
                               << " Etrue = " << mcgps[0]->getEnergy() << " VTX squared = " << rVertexSquared
                               << std::endl;
      }
    } else {
      ESum_Reco_Wrong += ggp->getEnergy();
      //      ESum_Reco_MC_Wrong += something    (need to find the true energies of the photons that have been wrongly
      //      paired up ...)
    }
    ...
    trueMassofSeen.push_back(
        sqrt(sumTrueE * sumTrueE - sumTrueP[0] * sumTrueP[0] - sumTrueP[1] * sumTrueP[1] - sumTrueP[2] * sumTrueP[2]));
  ...
  streamlog_out(DEBUG) << " SUMMARY = " << nMCPi0 << " ESum MCP = " << ESum_MCP << " " << nRecoPi0
                       ...
                       << " ESum Reco Wrong = " << ESum_Reco_Wrong << " ESum_Reco_MC_Correct " << ESum_Reco_MC_Correct
                       << " ESum_Reco_Meas " << ESum_Reco_Meas << " ESum_Reco_MC_All " << ESum_Reco_MC_All << std::endl;

  streamlog_out(DEBUG) << " SUMMARY2 = " << std::setw(2) << nMCPi0 << " " << std::setw(10) << ESum_MCP << " "
                       << std::setw(2) << nMCPi0 - nMCEta - nMCEtaPrime << " " << std::setw(2) << nMCEta << " "
                       << std::setw(2) << nMCEtaPrime << " " << std::setw(2) << nRecoPi0 << " " << std::setw(10)
                       << ESum_Reco << " " << std::setw(2) << nRecoPi0 - nRecoEta - nRecoEtaPrime << " " << std::setw(2)
                       << nRecoEta << " " << std::setw(2) << nRecoEtaPrime << " " << std::setw(2) << nCorrectPi0 << " "
                       << std::setw(2) << nCorrectEta << " " << std::setw(2) << nCorrectEtaPrime << " " << std::setw(10)
                       << ESum_Reco_Correct << " " << std::setw(10) << ESum_Reco_Wrong << " " << std::setw(10)
                       << ESum_Reco_MC_Correct << " " << std::setw(10) << ESum_Reco_Meas << " " << std::setw(10)
                       << ESum_Reco_MC_All << std::endl;
