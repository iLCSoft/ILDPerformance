...
TrackerHitCounter::TrackerHitCounter() : Processor("TrackerHitCounter"), _hitCounters(), m_trkHitCollNames() {
...
void TrackerHitCounter::init() {
  ...
  for (dd4hep::DetElement element : detElements) {
    ...
    streamlog_out(DEBUG7) << "Detector element \'" << element.name() << "\' of type \'" << element.type() << "\':\n";
...
void TrackerHitCounter::processEvent(LCEvent* evt) {
  ...
  for (auto collname : m_trkHitCollNames) {
    ...
    for (auto lcobj : (*col)) {
      ...
      if (!hit) {
        streamlog_out(WARNING) << "Collection " << collname
                               << " contains objects "
      ...
      if (hcmit == _hitCounters.end()) {
        ...
        if (shc->size() == 1) {
          ...
          if (hclit == shc->end()) {
            streamlog_out(ERROR) << "Hit in system ID=" << hcmit->first << " belongs to layer number " << nLayer
                                 << ". Out of range!\n"
...
void TrackerHitCounter::end() {
  ...
  for (dd4hep::DetElement element : detElements) {
    ...
    for (auto layerpair : *hitctr) {
      ...
      streamlog_out(MESSAGE) << "  Layer " << layerpair.first + 1 << ": " << layerctr->getNHits() << " hits.\n";
      streamlog_out(MESSAGE) << "    (" << layerctr->getHitsPerRun() << " +- " << layerctr->getStDevHitsPerRun()
                             << ") hits/run.\n";
      streamlog_out(MESSAGE) << "    (" << layerctr->getHitsPerEvent() << " +- " << layerctr->getStDevHitsPerEvent()
                             << ") hits/event.\n";
