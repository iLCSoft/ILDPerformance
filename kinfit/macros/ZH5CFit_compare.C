...
void ZH5CFit_compare(const char* reffilename = "ZH5CFit_OPAL.root",
                     const char* testfilename = "../rootfiles/ZH5CFit_test.root", const char* legtitle = "ZH -> 4jets",
  ...
  for (int ifile = 0; ifile < nfile; ++ifile) {
    ...
    for (int ihist = 0; ihist < nhist; ++ihist) {
      cout << "reading histogram " << histnames[ihist][ifile] << " from ifile " << ifile << " with name "
           << histfile[ifile]->GetName() << endl;
      ...
      if (h[ihist][ifile]->GetMinimum() > 1)
        h[ihist][ifile]->SetMinimum(1.);
      if (ifile == 0) { // test histo
        ...
        if (h[ihist][1]->GetMaximum() > max)
          max = h[ihist][1]->GetMaximum();
  ...
  // std::stTVirtualPad::SetOptFitring cFile1  ( std::string( "../plots/" )  +  std::string( outfilename ) +
  // std::string( "_1" ) + std::string( ".C" ) ) ;
  ...
  // std::string cFile2  ( std::string( "../plots/" )  +  std::string( outfilename ) + std::string( "_2" ) +
  // std::string( ".C" ) ) ;
  ...
  // std::string cFile3  ( std::string( "../plots/" )  +  std::string( outfilename ) + std::string( "_3" ) +
  // std::string( ".C" ) ) ;
