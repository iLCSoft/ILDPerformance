...
void draw_tof_singleparticle(
    ...
    const string InFileBase = "../../output/SingleParticle_rv02-00-01_sv02-00-01_mILD_l5_o1_v02/",
    const string OutFileBase = "../../output/SP") {
  ...
  int every_nth_bin =
      stoi(beta_p_settings["BinJumpsPerSlice"]); // Consider only every n-th bin (=slice) in the momentum distribution
  ...
  for (auto& ptype_name : ptype_names) {
    ...
    particle_types.push_back(ParticleType(stoi(ptype_sec["pdgID"]), ptype_sec["name_s"], ptype_name,
                                          stod(ptype_sec["mass"]), stoi(ptype_sec["colour"])));
  }
  ...
  unique_ptr<TCanvas> c_dummy{
      new TCanvas("dummy", "", 1000, 800)}; // To write histos to that don't need to be written to current canvas

  unique_ptr<TCanvas> c_beta_p{new TCanvas("c_beta_p", "#beta_{CH} vs momentum", 400 * N_ress, 400 * N_ptypes)};
  unique_ptr<TCanvas> c_beta_p_average{
      new TCanvas("c_beta_p_average", "#beta_{CH}^{average} vs momentum", 400 * N_ress, 400 * N_ptypes)};
  c_beta_p->Clear();
  ...
  for (auto& ptype : particle_types) {
    ...
    for (auto& res : resolutions) {
      ...
      string draw_string =
          tof_res + "len/" + tof_res + "ch/299.8:sqrt(rcmox*rcmox+rcmoy*rcmoy+rcmoz*rcmoz)>>" + h_beta_name;
      string cut_string = tof_res + "len>1.&&" + tof_res + "ch>0.1&&(" + tof_res + "len/" + tof_res +
                          "ch/299.8)<1.1 && sqrt(rcmox*rcmox+rcmoy*rcmoy+rcmoz*rcmoz)<100.&&abs(rccha)>0.005 && "
                          "abs(rcmoz)/sqrt(rcmox*rcmox+rcmoy*rcmoy+rcmoz*rcmoz) < 0.792573 && "
                          "sqrt(rcmox*rcmox+rcmoy*rcmoy) > 0.948543 && nrec==1";
      ...
      TH1F* h_beta_average = new TH1F(h_beta_average_name.c_str(),
                                      ("#beta_{CH}^{average} vs momentum, " + ptype.name_l + ", " + res.w_unit() +
                                       ", error = #sigma_{gaus}; p [GeV/c] ; #beta ")
                                          .c_str(),
                                      bins_p, min_p, max_p);
  ...
  for (int i = 0; i < N_ptypes - 1; i++) {
    ...
    for (int j = i + 1; j < N_ptypes; j++) { // Loop avoids dublicats
      ...
      unique_ptr<TCanvas> c_sep_pwr{new TCanvas(("c_sep_pwr_" + ptype1.name_s + "_" + ptype2.name_s).c_str(),
                                                "Separation power vs momentum", 800 * resolutions.size(), 800)};
      ...
      for (auto& res : resolutions) {
        ...
        TH1D* h_seppwr = new TH1D(("h_seppwr_" + ptype1.name_s + ptype2.name_s + res.str()).c_str(),
                                  (ptype1.name_l + " - " + ptype2.name_l + " TOF-separation, " + res.w_unit() +
                                   "; p [GeV/c] ; Separation Power [#sigma]")
                                      .c_str(),
        ...
        for (int i_slice = 0; i_slice < N_slices; i_slice++) {
          ...
          if (!(slice_p1.fit_successful && slice_p2.fit_successful)) {
            cout << ptype1.name_l << " - " << ptype2.name_l << ", " << res.w_unit() << " : Fit in slice p=" << p
                 << " not successful, point won't be drawn." << endl;
            continue;
          }
          ...
          double sep_pwr_err = sqrt((pow(slice_p1.mean_err, 2) + pow(slice_p2.mean_err, 2)) / ms_sigma +
                                    pow(diff_mean, 2) *
                                        (pow(slice_p1.sigma, 2) * pow(slice_p1.sigma_err, 2) +
                                         pow(slice_p2.sigma, 2) * pow(slice_p2.sigma_err, 2)) /
                                        (4 * pow(ms_sigma, 3)));
          // Check that sep pwr is not NaN
          if (sep_pwr != sep_pwr) {
            sep_pwr = sep_pwr_err = 0;
          }
      ...
      unique_ptr<TCanvas> c_sep_pwr_comb{new TCanvas(("c_sep_pwr_comb_" + ptype1.name_s + "_" + ptype2.name_s).c_str(),
                                                     "Separation power vs momentum", 800, 800)};
      ...
      if (h_sep_pwr_comb->GetMaximum() > 120) {
        h_sep_pwr_comb->SetMaximum(100);
      }
      gPad->Modified();
      gPad->Update();
