struct ParticleType {
  ...
  ParticleType(const ParticleType& _ptype)
      : pdg_id(_ptype.pdg_id), name_s(_ptype.name_s), name_l(_ptype.name_l), mass(_ptype.mass),
        colour_index(_ptype.colour_index) {};
...
struct Resolution {
  ...
  bool operator==(const Resolution& res) { return value == res.value && unit == res.unit; }
  ...
  string w_unit() const { return this->str() + unit; }
};
...
struct Histo {
  ...
  void slice_histo(int bin_step_size, bool do_fit = true) {
    ...
    for (int slice_bin = 1; slice_bin < histo->GetNbinsX() + 1; slice_bin += bin_step_size) {
      ...
      slice.histo->SetTitle(
          TString("#beta_{CH} sliced at p=" + to_string(p) + "GeV, bin " + to_string(slice_bin) + ", " + ptype.name_l));
...
struct HistoMap {
  ...
  void addHisto(ParticleType& _ptype, Resolution& _res) { histos.push_back(make_shared<Histo>(_ptype, _res)); }

  HistoPtrVec getHistosToRes(Resolution& res) const {
    HistoPtrVec histos_to_res{};
    for (auto& histo : histos) {
      if (histo->res == res) {
        histos_to_res.push_back(histo);
      }
    }
  ...
  HistoPtrVec getHistosToPType(ParticleType& ptype) const {
    ...
    for (auto& histo : histos) {
      if (histo->ptype == ptype) {
        histos_to_ptype.push_back(histo);
      }
    }
