...
void draw_tof_ttbar(const string InFile = "lctuplep.root", const string OutFileBase = "ttbar") {
  ...
  for (auto& res : resolutions) {
    ...
    unique_ptr<TCanvas> c_beta_p{new TCanvas(("c_beta_p_" + res.str()).c_str(),
                                             ("#beta vs momentum, ttbar, " + res.w_unit()).c_str(), 500,
                                             500)}; //(plot, name, [ 10, -5, -10 100 all looks same] )
    c_beta_p->Clear();

    string h_beta_name = "h_beta_" + res.str();
    TH2F* h_beta = new TH2F(h_beta_name.c_str(), ("#beta vs momentum, ttbar, " + res.w_unit()).c_str(), 200, 0, 10.,
                            200, 0.5, 1.); // (plot ,name ,nbin x ,x low ,x up ,nbin y ,y low ,y up)
    ...
    string cut_string = tof_res + "len>1.&&" + tof_res + "ch>0.1&&(" + tof_res + "len/" + tof_res +
                        "ch/299.8)<1.1 && sqrt(rcmox*rcmox+rcmoy*rcmoy+rcmoz*rcmoz)<10.&&abs(rccha)>0.01";
    ...
    unique_ptr<TCanvas> c_beta_p_slices{new TCanvas(("c_beta_p_slices_" + res.str()).c_str(),
                                                    ("#beta, p slices, ttbar, " + res.w_unit()).c_str(), 500 * N_slices,
                                                    500)}; //(plot, name, [ 10, -5, -10 100 all looks same] )
  ...
  for (int i = 0; i < resolutions.size() - 1; i++) {
    ...
    for (int j = i + 1; j < resolutions.size(); j++) {
      ...
      for (auto& method : tof_methods) {
        unique_ptr<TCanvas> c_tof_diff{new TCanvas(
            ("c_tof_diff_" + res_1.str() + res_2.str() + method.name_s).c_str(),
            ("TOF difference " + method.name_l + ", " + res_1.w_unit() + "-" + res_2.w_unit()).c_str(), 500, 500)};
        c_tof_diff->Clear();
        string h_diff_name = "h_tof_diff_" + res_1.str() + res_2.str() + method.name_s;
        TH1D* h_tof_diff = new TH1D(
            h_diff_name.c_str(),
            ("#Delta TOF_{" + method.name_s + "}, " + res_1.w_unit() + "-" + res_2.w_unit() + "; #Delta TOF; #")
                .c_str(),
            1000, -1., 1.);

        string draw_string =
            "tof" + res_1.str() + method.name_s + "-tof" + res_2.str() + method.name_s + ">>" + h_diff_name;
        string cut_string =
            "tof" + res_1.str() + method.name_s + " != -1 && tof" + res_2.str() + method.name_s + " != -1";
        tree->Draw(draw_string.c_str(), cut_string.c_str());
  ...
  for (auto& res : resolutions) {
    ...
    for (auto& ptype : particle_types) {
      ...
      TH2F* h_beta_p =
          new TH2F(("ttbar_beta_p_" + ptype.name_s + "_" + res.str()).c_str(),
                   ("#beta vs p, " + ptype.name_l + ", " + res.w_unit() + "; p; #beta; #" + ptype.name_l).c_str(), 200,
                   0, 10., 200, 0.5, 1.05);
        ...
        for (int i_rec = 0; i_rec < N_rec; i_rec++) {
          ...
          if (weight > 0.9) {
            ...
            if (final_i_rel < 0 || final_i_rel >= r2mnrel) {
              cout << "ERROR: MCParticle index out of range: final = " << final_i_rel << ", r2mnrel = " << r2mnrel
                   << endl;
              continue;
            }
            ...
            if (mc_is_ptype) {
              ...
              if (beta < 1.1 && p < 10. && tof_len[i_rec] > 1. && tof_ch[i_rec] > 0.1 && abs(rccha[i_rec]) > 0.01 &&
                  abs(rcmoz[i_rec]) / sqrt(rcmox[i_rec] * rcmox[i_rec] + rcmoy[i_rec] * rcmoy[i_rec] +
                                           rcmoz[i_rec] * rcmoz[i_rec]) >
                      0.79257283) {
