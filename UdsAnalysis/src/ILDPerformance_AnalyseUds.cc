...
public:
   ...
   *  @param  print
   */
  static void CalculatePerformance(const TH1F* const pTH1F, float& resolution, float& resolutionError, float& scale,
                                   float& scaleError, bool print = true);
};
...
void AnalysisHelper::CalculatePerformance(const TH1F* const pTH1F, float& resolution, float& resolutionError,
                                          float& scale, float& scaleError, bool print) {
  ...
  float rmsmin(FLOAT_MAX), sigma(FLOAT_MAX), sigmasigma(FLOAT_MAX), frac(FLOAT_MAX), efrac(FLOAT_MAX), mean(FLOAT_MAX),
      low(FLOAT_MAX), rms(FLOAT_MAX);
  float high(0.f);
  ...
  if (print) {
    std::cout << pTH1F->GetName() << " (" << pTH1F->GetEntries() << " entries), rawrms: " << rawRms
              << ", rms90: " << rmsmin << " (" << low << "-" << high << "), mean: " << mean << ", sigma: " << sigma
              << "+-" << sigmasigma;
...
UdsAnalysis::UdsAnalysis()
    : m_filePattern(""), m_trueEnergies(), m_trueEnergiesStr(""), m_outputFile("ILDPerformance_UdsAnalysis.root"),
      m_treeName("PfoAnalysisTree"), m_withoutNeutrinoEnergy(false) {}
...
void UdsAnalysis::Process() {
  ...
  TGraphErrors* fJer = new TGraphErrors(m_trueEnergies.size());
  deletionPool.push_back(fJer);
  fJer->SetMarkerSize(3);
  fJer->SetLineWidth(2);
  fJer->SetTitle("Jet energy resolution;Jet energy [GeV];RMS_{90}(E_{j}) / Mean_{90}(E_{j}) [%]");

  TGraphErrors* fJerL7A = new TGraphErrors(m_trueEnergies.size());
  deletionPool.push_back(fJerL7A);
  fJerL7A->SetMarkerSize(3);
  fJerL7A->SetLineWidth(2);
  fJerL7A->SetTitle("Jet energy resolution, cos(#theta) < 0.7;Jet energy [GeV];RMS_{90}(E_{j}) / Mean_{90}(E_{j}) [%]");

  TGraphErrors* fJerH7A = new TGraphErrors(m_trueEnergies.size());
  deletionPool.push_back(fJerH7A);
  fJerH7A->SetMarkerSize(3);
  fJerH7A->SetLineWidth(2);
  fJerH7A->SetTitle("Jet energy resolution, cos(#theta) > 0.7;Jet energy [GeV];RMS_{90}(E_{j}) / Mean_{90}(E_{j}) [%]");

  // Jet energy scale plots
  TGraphErrors* fJes = new TGraphErrors(m_trueEnergies.size());
  deletionPool.push_back(fJes);
  fJes->SetTitle("Jet energy scale;Jet energy [GeV];Reconstructed jet energy [GeV]");
  fJes->SetMarkerSize(3);
  fJes->SetLineWidth(1);
  fJes->SetDrawOption("");
  TF1* pXEqualY = new TF1("PerfectJes", "x", 0, lastEnergy + 10);
  fJes->GetListOfFunctions()->Add(pXEqualY);

  TGraphErrors* fJesL7A = new TGraphErrors(m_trueEnergies.size());
  deletionPool.push_back(fJesL7A);
  fJesL7A->SetTitle("Jet energy scale, cos(#theta) < 0.7;Jet energy [GeV];Reconstructed jet energy [GeV]");
  fJesL7A->SetMarkerSize(3);
  fJesL7A->SetLineWidth(1);
  TF1* pXEqualYL7A = new TF1("PerfectJesL7A", "x", 0, lastEnergy + 10);
  fJesL7A->GetListOfFunctions()->Add(pXEqualYL7A);

  TGraphErrors* fJesH7A = new TGraphErrors(m_trueEnergies.size());
  deletionPool.push_back(fJesH7A);
  fJesH7A->SetTitle("Jet energy scale, cos(#theta) > 0.7;Jet energy [GeV];Reconstructed jet energy [GeV]");
  fJesH7A->SetMarkerSize(3);
  fJesH7A->SetLineWidth(1);
  TF1* pXEqualYH7A = new TF1("PerfectJesH7A", "x", 0, lastEnergy + 10);
  fJesH7A->GetListOfFunctions()->Add(pXEqualYH7A);
  ...
  for (unsigned int e = 0; e < m_trueEnergies.size(); e++) {
    ...
    // Book histograms
    TH1F* pResVsCosThetaHist =
        new TH1F(("ResVsCosTheta" + energyStr + "GeV").c_str(),
                 ("RMS_{90}(E_{j}) / Mean_{90}(E_{j}) vs |cos(#theta)| (" + energyStr + "GeV)").c_str(), nRegionBins,
                 pRegionBinEdges);
    pResVsCosThetaHist->SetYTitle("RMS_{90}(E_{j}) / Mean_{90}(E_{j}) [%]");
    pResVsCosThetaHist->SetXTitle("|cos(#theta)|");

    TH1F* pRecVsCosThetaHist =
        new TH1F(("RecVsCosTheta" + energyStr + "GeV").c_str(),
                 ("Mean_{90}(E_{j}) vs |cos(#theta)| (" + energyStr + "GeV)").c_str(), nRegionBins, pRegionBinEdges);
    ...
    TH1F* pPFA = new TH1F(("fPFA_" + energyStr).c_str(), ("TotalEnergy (" + energyStr + "GeV)").c_str(), nEnergyBins,
                          energyRangeMin, energyRangeMax);
    TH1F* pPFAL7A = new TH1F(("fPFA_L7A_" + energyStr).c_str(), ("TotalEnergy<0.7A (" + energyStr + "GeV)").c_str(),
                             nEnergyBins, energyRangeMin, energyRangeMax);
    TH1F* pPFAH7A = new TH1F(("fPFA_H7A_" + energyStr).c_str(), ("TotalEnergy>0.7A (" + energyStr + "GeV)").c_str(),
                             nEnergyBins, energyRangeMin, energyRangeMax);
    ...
    for (unsigned int i = 0; i < nRegionBins; ++i) {
      ...
      pRegionHistograms[i] =
          new TH1F(name.str().c_str(), title.str().c_str(), nEnergyBins, energyRangeMin, energyRangeMax);
    }
    ...
    for (unsigned int i = 0; i < nRegionBins; ++i) {
      ...
      pRecVsCosThetaHist->SetBinContent(i + 1, scale);
      pRecVsCosThetaHist->SetBinError(i + 1, scaleError);
      pResVsCosThetaHist->SetBinContent(i + 1, resolution);
      pResVsCosThetaHist->SetBinError(i + 1, resolutionError);
    }
  ...
  for (std::vector<TObject*>::iterator iter = deletionPool.begin(), endIter = deletionPool.end(); endIter != iter;
       ++iter)
    delete *iter;
...
bool ParseCommandLine(int argc, char* argv[], UdsAnalysis& udsAnalysis) {
  ...
  while (((c = getopt(argc, argv, "o:e:f:t:n")) != -1) || (argc == 1)) {
    ...
    default:
      std::cout << std::endl
                << "Usage: ILDPerformance_UdsAnalysis " << std::endl
                << "    -o file      (optional,  output file to write results to. Default is "
                   "'ILDPerformance_UdsAnalysis.root')          "
                << std::endl
                << "    -e energies  (mandatory, input energies column separated, i.e '40:91:200:360:500')             "
                   "                  "
                << std::endl
                << "    -f pattern   (mandatory, input file pattern. Should contains %{energy} key, i.e "
                   "./PfoAnalysisUds_%{energy}.root) "
                << std::endl
                << "    -t treename  (optional,  the tree name to look in root files, usually PfoAnalysisTree)         "
                   "                  "
                << std::endl
                << "    -n           (optional,  whether to do not include MC neutrino energy in total energy "
