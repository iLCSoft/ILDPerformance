...
validateAutoProcessor::validateAutoProcessor() : Processor("validateAutoProcessor") {
  ...
  registerProcessorParameter("inputParamFilenames", "names of input parameter files", _infiles, infiles);

  registerProcessorParameter("outputFilename", "name of output file", _outfile, std::string("validate.root"));
}
...
void validateAutoProcessor::init() {
  ...
  for (std::map<std::string, validatePilotProcessor_maxMin>::iterator itt = _allranges.begin(); itt != _allranges.end();
    ...
    cout << itt->first << " is endcap ? " << itt->second.abszminmax.first << " " << itt->second.zminmax.second << " "
         << itt->second.zminmax.second / 15. << " " << isEndcap << endl;
    ...
    for (size_t jj = 0; jj < itt->second.indx_name.size(); jj++) {
      hname = itt->first.c_str();
      hname += "_Indx" + itt->second.indx_name[jj] + "_X";
      hname1 = isEndcap ? hname + "_posZ" : hname;
    ...
    for (size_t jj = 0; jj < itt->second.indx_name.size(); jj++) {
      hname = itt->first.c_str();
      hname += "_Indx" + itt->second.indx_name[jj] + "_Y";
      hname1 = isEndcap ? hname + "_posZ" : hname;
      hmin = itt->second.indx_minmax[jj].first - 1;
      hmax = itt->second.indx_minmax[jj].second + 2;
      nbins = hmax - hmin;
    ...
    for (size_t jj = 0; jj < itt->second.indx_name.size(); jj++) {
      hname = itt->first.c_str();
      hname += "_Indx" + itt->second.indx_name[jj] + "_Z";
      hname1 = isEndcap ? hname + "_posZ" : hname;
      hmin = itt->second.indx_minmax[jj].first - 1;
      hmax = itt->second.indx_minmax[jj].second + 2;
      nbins = hmax - hmin;
    ...
    for (size_t jj = 0; jj < itt->second.indx_name.size(); jj++) {
      hname = itt->first.c_str();
      hname += "_Indx" + itt->second.indx_name[jj] + "_R";
      hname1 = isEndcap ? hname + "_posZ" : hname;
      hmin = itt->second.indx_minmax[jj].first - 1;
      hmax = itt->second.indx_minmax[jj].second + 2;
      nbins = hmax - hmin;
    ...
    for (size_t jj = 0; jj < itt->second.indx_name.size(); jj++) {
      hname = itt->first.c_str();
      hname += "_Indx" + itt->second.indx_name[jj] + "_Phi";
      hname1 = isEndcap ? hname + "_posZ" : hname;
      hmin = itt->second.indx_minmax[jj].first - 1;
      hmax = itt->second.indx_minmax[jj].second + 2;
      nbins = hmax - hmin;
  ...
  _hAll_overallposZR =
      new TH2F(hname1, hname1, 2 * nbins2d, -zmaxall * 1.1, zmaxall * 1.1, 2 * nbins2d, 0, rmaxall * 1.1);
  _hAll_overallposZR->GetXaxis()->SetTitle("Z [mm]");
  _hAll_overallposZR->GetYaxis()->SetTitle("R [mm]");
  hname1 = "ALLCollections_Log_overallhitZR";
  _hAll_Log_overallposZR =
      new TH2F(hname1, hname1, 2 * nbins2d, -1, log10(zmaxall) * 1.1, 2 * nbins2d, 0.5, log10(rmaxall) * 1.1);
...
void validateAutoProcessor::end() {
  std::cout << "validateAutoProcessor::end()  " << name() << std::endl;
