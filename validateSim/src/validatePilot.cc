...
validatePilotProcessor::validatePilotProcessor() : Processor("validationPreProcessor") {
  ...
  registerProcessorParameter("outputParamFilename", "name of output parameter file", _outfile, std::string("BLAH"));
}
      ...
      for (int jj = 0; jj < col->getNumberOfElements(); jj++) {
        ...
        if (_allranges.find(colname) == _allranges.end()) {
          ...
          for (size_t j = 0; j < times.size(); j++) {
            if (times[j] < vmm.tminmax.first)
              vmm.tminmax.first = times[j];
            if (times[j] > vmm.tminmax.second)
              vmm.tminmax.second = times[j];
          }
          ...
          for (size_t j = 0; j < indices.size(); j++) {
            if (indices[j] < _allranges[colname].indx_minmax[j].first)
              _allranges[colname].indx_minmax[j].first = indices[j];
            if (indices[j] > _allranges[colname].indx_minmax[j].second)
              _allranges[colname].indx_minmax[j].second = indices[j];
          }
          if (energy < _allranges[colname].eminmax.first)
            _allranges[colname].eminmax.first = energy;
          if (energy > _allranges[colname].eminmax.second)
            _allranges[colname].eminmax.second = energy;
          for (size_t j = 0; j < times.size(); j++) {
            if (times[j] < _allranges[colname].tminmax.first)
              _allranges[colname].tminmax.first = times[j];
            if (times[j] > _allranges[colname].tminmax.second)
              _allranges[colname].tminmax.second = times[j];
          }
          if (pos[0] < _allranges[colname].xminmax.first)
            _allranges[colname].xminmax.first = pos[0];
          if (pos[0] > _allranges[colname].xminmax.second)
            _allranges[colname].xminmax.second = pos[0];

          if (pos[1] < _allranges[colname].yminmax.first)
            _allranges[colname].yminmax.first = pos[1];
          if (pos[1] > _allranges[colname].yminmax.second)
            _allranges[colname].yminmax.second = pos[1];

          if (pos[2] < _allranges[colname].zminmax.first)
            _allranges[colname].zminmax.first = pos[2];
          if (pos[2] > _allranges[colname].zminmax.second)
            _allranges[colname].zminmax.second = pos[2];

          float radius = sqrt(pos[0] * pos[0] + pos[1] * pos[1]);
          float absz = fabs(pos[2]);

          if (radius < _allranges[colname].rminmax.first)
            _allranges[colname].rminmax.first = radius;
          if (radius > _allranges[colname].rminmax.second)
            _allranges[colname].rminmax.second = radius;

          if (absz < _allranges[colname].abszminmax.first)
            _allranges[colname].abszminmax.first = absz;
          if (absz > _allranges[colname].abszminmax.second)
            _allranges[colname].abszminmax.second = absz;
        }
...
void validatePilotProcessor::end() {
  std::cout << "validatePilotProcessor::end()  " << name() << std::endl;
  ...
  for (std::map<std::string, validatePilotProcessor_maxMin>::iterator itt = _allranges.begin(); itt != _allranges.end();
       itt++) {
    ...
    label = itt->first + " Energy";
    myfile[ttt] << label << " " << vmm.eminmax.first << " " << vmm.eminmax.second << endl;
    label = itt->first + " Time";
    myfile[ttt] << label << " " << vmm.tminmax.first << " " << vmm.tminmax.second << endl;
    label = itt->first + " X";
    myfile[ttt] << label << " " << vmm.xminmax.first << " " << vmm.xminmax.second << endl;
    label = itt->first + " Y";
    myfile[ttt] << label << " " << vmm.yminmax.first << " " << vmm.yminmax.second << endl;
    label = itt->first + " Z";
    myfile[ttt] << label << " " << vmm.zminmax.first << " " << vmm.zminmax.second << endl;
    label = itt->first + " Absz";
    myfile[ttt] << label << " " << vmm.abszminmax.first << " " << vmm.abszminmax.second << endl;
    label = itt->first + " R";
    myfile[ttt] << label << " " << vmm.rminmax.first << " " << vmm.rminmax.second << endl;
  }
