      ...
      while ((key2 = (TKey*)next2())) {
        ...
        if (cll->InheritsFrom("TH2F")) {
          ...
          if (hn.Contains("Indx")) {
            ...
            if (find(indices.begin(), indices.end(), asas) == indices.end())
              indices.push_back(asas);
            asas = ((TObjString*)(ss.Tokenize("_")->At(1)))->GetString();
            if (find(variables.begin(), variables.end(), asas) == variables.end())
              variables.push_back(asas);
          }
        }
      }

      if (indices.size() == 0 || variables.size() == 0)
        continue;
      ...
      for (int inp = 0; inp < 2; inp++) {
        ...
        while ((key2 = (TKey*)next2())) {
          ...
          if (cll->InheritsFrom("TH2F")) {
            ...
            TString hn = hh->GetName();
            if (isEndcap) {
              if (inp == 0 && !hn.Contains("posZ"))
                continue;
              else if (inp == 1 && !hn.Contains("negZ"))
                continue;
            }
            if (hn.Contains("Indx")) {
        ...
        if (isEndcap) {
          if (inp == 0)
            tt.DrawTextNDC(0.35, 0.99, "posZ");
          else
            tt.DrawTextNDC(0.35, 0.99, "negZ");
        }
