...
void WWZZProcessor::fillVPairInformation(std::vector<LCIOParticleType*> V1, std::vector<LCIOParticleType*> V2,
                                         Observables& observ) {
...
void WWZZProcessor::getAllVVTo4ParticleCombinations(
    std::vector<LCIOParticleType*> particles, std::vector<std::vector<std::vector<LCIOParticleType*>>>& VV_candidates) {
...
template <typename LCIOParticleType>
void WWZZProcessor::getVVCandidatesMasses(std::vector<std::vector<std::vector<LCIOParticleType*>>> VV_candidates,
...
int WWZZProcessor::findIndexBestPairInVVCandidates(
  ...
  */
  ...
  int best_index =
      std::distance(std::begin(minimize_me), std::min_element(std::begin(minimize_me), std::end(minimize_me)));
  return best_index;
}
...
void WWZZProcessor::findVpair(std::vector<LCIOParticleType*> particles, Observables& observ) {
  ...
  streamlog_out(DEBUG5) << "\nResulted best mass difference:" << fabs(observ.pair1_mass - observ.pair2_mass)
                        << std::endl;
}
