...
public:
  ...
  /** Subroutines
   */
  virtual void analyseEvent(LCCollection* colMC, LCCollection* colPFOs, LCCollection* colJets, LCCollection* colIsoLeps,
                            EventInfo& info);
  ...
  virtual void findAllowedVVCandidates(std::vector<std::vector<std::vector<MCParticle*>>> all_VV_candidates,
  ...
  void fillVPairInformation(std::vector<LCIOParticleType*> V1, std::vector<LCIOParticleType*> V2, Observables& observ);

  template <typename LCIOParticleType>
  void getAllVVTo4ParticleCombinations(std::vector<LCIOParticleType*> particles,
                                       std::vector<std::vector<std::vector<LCIOParticleType*>>>& VV_candidates);

  template <typename LCIOParticleType>
  void getVVCandidatesMasses(std::vector<std::vector<std::vector<LCIOParticleType*>>> VV_candidates,
                             std::vector<std::vector<float>>& candidates_masses);

  template <typename LCIOParticleType>
  int findIndexBestPairInVVCandidates(std::vector<std::vector<std::vector<LCIOParticleType*>>> VV_candidates);

  template <typename LCIOParticleType>
  void findVpair(std::vector<LCIOParticleType*> jets, Observables& observ);

  // Cut parameter calculation
  virtual void calculateCutParameters(std::vector<ReconstructedParticle*> PFOs,
                                      std::vector<ReconstructedParticle*> jets, Observables& observ);
