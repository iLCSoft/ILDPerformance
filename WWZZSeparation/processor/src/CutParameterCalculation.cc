...
void WWZZProcessor::calculateCutParameters(std::vector<ReconstructedParticle*> PFOs,
  ...
  */
  ...
  for (int i = 0; i < nJets; i++) {
    ...
    if (min_Ejet > jet_tlv.E()) {
      min_Ejet = jet_tlv.E();
    }
    if (min_num_particles_jet > nJetPFOs) {
      min_num_particles_jet = nJetPFOs;
    }
