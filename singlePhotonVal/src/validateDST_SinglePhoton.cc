...
validateDST_SinglePhotonProcessor::validateDST_SinglePhotonProcessor()
    : Processor("validateDST_SinglePhotonProcessor") {

  // modify processor description
  _description = "validateDST_SinglePhotonProcessor makes histograms of basic properties for photons";

  registerProcessorParameter("outputFilename", "name of output file", _outfile, std::string("validatePhoton.root"));
}
...
void validateDST_SinglePhotonProcessor::processEvent(LCEvent* evt) {
  ...
  try {
    ...
    if (enHistos.find(imcenergy) == enHistos.end()) {
      ...
      histos2d[ivar_barenPhi] =
          new TH2F(hnn + "enBarrelPhi", hnn + "enBarrelPhi", 100, -0.05, 3.141 / 4 + 0.05, 1000, 0, emax);
      histos2d[ivar_bartotenPhi] =
          new TH2F(hnn + "totenBarrelPhi", hnn + "totenBarrelPhi", 100, -0.05, 3.141 / 4 + 0.05, 1000, 0, emax);
...
void validateDST_SinglePhotonProcessor::end() {
  std::cout << "validateDST_SinglePhotonProcessor::end()  " << name() << std::endl;
