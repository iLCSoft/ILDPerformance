...
struct WSort { // sort track segments wtr to higher inverse relative weight
  inline bool operator()(const std::pair<EVENT::MCParticle*, EVENT::LCRelation*>& l,
                         const std::pair<EVENT::MCParticle*, EVENT::LCRelation*>& r) {
...
struct invWSort { // sort track segments wtr to higher inverse relative weight
  inline bool operator()(const std::pair<EVENT::Track*, EVENT::LCRelation*>& l,
                         const std::pair<EVENT::Track*, EVENT::LCRelation*>& r) {
    return (fabs((l.second)->getWeight()) > fabs((r.second)->getWeight()));
  }
};
//-------------------------------------------------------------------------------------------------------------
...
#define APPLY_CUT(LEVEL, Cut, Exp)                                                                                     \
  if ((Exp) == false) {                                                                                                \
    if (Cut) {                                                                                                         \
      streamlog_out(LEVEL) << "  ***** failed cut:  [ " << #Exp << " ] in evt: " << evt->getEventNumber()              \
...
void DDDiagnostics::clearVec(void) {
  ...
  recoD0Error.clear();
  recoZ0Error.clear();
  recoOmegaError.clear();
  recoPhiError.clear();
  recoTanLambdaError.clear();
...
void DDDiagnostics::initParameters(void) {
  ...
  registerInputCollection(LCIO::LCRELATION, "MCTracksTruthLinkCollectionName", "true - reco relation collection",
                          _trueToReco, std::string("MCTracksTruthLink"));

  registerInputCollection(LCIO::LCRELATION, "TracksMCTruthLinkCollectionName", "reco - true relation collection",
                          _recoToTrue, std::string("TracksMCTruthLink"));

  registerInputCollection(LCIO::LCRELATION, "SITTrackerHitRelations", "SIT tracker hits to sim hits",
                          _sitTrkHitRelations, std::string("SITTrackerHitRelations"));

  registerInputCollection(LCIO::LCRELATION, "VXDTrackerHitRelations", "VXD tracker hits to sim hits",
                          _vxdTrkHitRelations, std::string("VXDTrackerHitRelations"));
  ...
  registerInputCollections(LCIO::SIMTRACKERHIT, "SimTrackerHitCollections",
                           "Names of the SimTrackerHits input collection", _simTrkHitCollectionNames, exampleSimHits);

  registerInputCollection(LCIO::TRACKERHIT, "SpcPntsCollection", "Name of the SIT SpacePoints collection",
                          _sitTrackerHits, std::string("SITTrackerHits"));

  registerInputCollection(LCIO::TRACKERHIT, "VXDTrackerHits", "Name of the VXDTrackerHit collection", _vxdTrackerHits,
                          std::string("VXDTrackerHits"));

  registerInputCollection(LCIO::TRACK, "StudiedTracks", "Name of the FullLDC track collection", _trackColName,
                          std::string("MarlinTrkTracks"));

  registerProcessorParameter("TrkEffOn", "Enables cuts to define the examined track sample", _trkEffOn, bool(false));

  registerProcessorParameter("SiTrkEffOn", "Enables cuts to define the examined track sample", _siTrkEffOn,
                             bool(false));

  registerProcessorParameter("PhysSampleOn", "Enable when we run diagnostics to a simulated physics sample",
                             _physSampleOn, bool(false));

  registerProcessorParameter("CosThetaCut", "Maximum costheta of examined MCParticles sample", _cosTheta, double(0.9));

  registerProcessorParameter("PCut", "Minimum momentum of examined MCParticles sample (in GeV)", _pCut, double(0));

  registerProcessorParameter("PTCut", "Minimum transverse momentum of examined MCParticles sample (in GeV)", _ptCut,
                             double(0));

  registerProcessorParameter("DistFromIP", "Maximum distance from IP of examined MCParticles sample (in mm)",
                             _originCut, double(10.0));
  ...
  registerProcessorParameter("RequiredPurity",
                             ...
                             _minPurity, double(0.74));

  registerProcessorParameter("RequiredSiHits",
                             "minimum required hits on Si detectors in order to consider the track found", _minSiHits,
                             int(4));

  registerProcessorParameter("ReqInnVXDHit", "Requirement that the innermost VXD hit should be found", _reqInnVXDHit,
                             bool(false));
...
void DDDiagnostics::processEvent(LCEvent* evt) {
  ...
  int flagTrack = 0;
  int flagRecoToTrue = 0;
  int flagTrueToReco = 0;
  ...
  if (flagRecoToTrue == 1 && flagTrueToReco == 1) { // condition for the existence of the relation collections
    ...
    for (unsigned i = 0, iN = _simTrkHitCollectionNames.size(); i < iN; ++i) {
      ...
      try {
        col = evt->getCollection(_simTrkHitCollectionNames[i]);
      ...
      if (col) {
        ...
        if (_simTrkHitCollectionNames[i] == "VXDCollection" ||
          ...
          for (int j = 0, jN = col->getNumberOfElements(); j < jN; ++j) {
            ...
            streamlog_out(DEBUG2) << " MC particle " << mcp << " creates hit " << simHit << " in VXD layer " << layer
                                  << std::endl;
        ...
        if (_simTrkHitCollectionNames[i] == "FTDCollection" ||
          ...
          for (int j = 0, jN = col->getNumberOfElements(); j < jN; ++j) {
            ...
            streamlog_out(DEBUG2) << " MC particle " << mcp << " creates hit " << simHit << " in FTD layer " << layer
                                  << std::endl;
    ...
    for (std::map<MCParticle*, std::map<int, int>>::iterator ii = HitsInLayersPerMCP_VXD.begin();
         ii != HitsInLayersPerMCP_VXD.end(); ++ii) {
      streamlog_out(DEBUG3) << "TEST: mcp " << (*ii).first << " has hitted " << ((*ii).second).size() << " layers "
                            << std::endl;
    ...
    while (MCParticle* mcp = mcpIt.next()) {
      ...
      streamlog_out(DEBUG2) << " mcparticle " << mcp << " VXD hits " << hitMapVXD[mcp] << " SIT hits " << hitMapSIT[mcp]
                            << " TPC hits " << hitMapTPC[mcp] << std::endl;

      bool cut = true;

      if (fabs(mcp->getCharge()) < 0.5)
        cut = false; // silent cut, must be charge particles.
      if (cut)
      ...
      if (_trkEffOn) {
        ...
        if (_minSiHits > 0)
          APPLY_CUT(DEBUG, cut, SiHits_mcp >= _minSiHits);

        if (_siTrkEffOn && _reqInnVXDHit)
          APPLY_CUT(DEBUG, cut, hitMapVXD[mcp] >= _minSiHits);

        // APPLY_CUT( DEBUG, cut, hitMapVXD[ mcp ]  > 5  && noOfLayersPerMCP[ mcp ] > 5  ) ; //  Accept as examined
        // track subsample the MCParticles that creates certain number of hits to each subdetector
      ...
      if (cut) {
        ...
        streamlog_out(DEBUG4) << " mcparticle " << mcp << " makes hits in " << noOfLayersPerMCP[mcp] << " VXD layers "
                              << std::endl;
      }
    }
    ...
    for (int ii = 0, nn = mcpTracks.size(); ii < nn; ++ii) {
      ...
      streamlog_out(DEBUG4) << " Checking: pt mcp = " << ptmcp << " of particle " << mcpTracks[ii]
                            << " no of tracks associated " << testFromWgt.size() << std::endl;
      ...
      for (unsigned int jj = 0; jj < trkvec.size(); jj++) {
        ...
        streamlog_out(DEBUG4) << " MCParticle " << mcpTracks[ii] << " index " << jj << " track " << trkvec[jj]
                              ...
                              << TotalHits << " Total hits in fit " << TotalHitsInFit << std::endl;
        streamlog_out(DEBUG4) << " Reco track " << ((Track*)trkvec[jj]) << "  Pt "
                              << (fabs(1. / (((Track*)trkvec[jj])->getOmega())) / 1000.0)
                              << " Inverse relative weight to MC particle " << testWgt[jj] << " purity "
                              << testFromWgt[jj] << " Silicon hits " << SiHits << std::endl;
        ...
        if (_siTrkEffOn) {
          ...
          if (_reqInnVXDHit) {
            if (testFromWgt[jj] > _minPurity && SiHits >= _minSiHits && IPFlag == 1) {
              foundFlag = 1;
            }
          } else {
            if (testFromWgt[jj] > _minPurity && SiHits >= _minSiHits) {
              foundFlag = 1;
            }
          }
        } else {
          if (testFromWgt[jj] > _minPurity) {
            foundFlag = 1;
          }
        ...
        gear::Vector3D p2(mcpTracks[ii]->getMomentum()[0], mcpTracks[ii]->getMomentum()[1],
                          mcpTracks[ii]->getMomentum()[2]);
        ...
        if (foundFlag == 1) {
          ...
          if (foundTrkNdf != 0) {
            foundTrkChi2OverNdof.push_back(foundTrkChi2 / foundTrkNdf);
          }
        }
      ...
      if (foundFlag == 1) {
        streamlog_out(DEBUG4) << " Particle " << mcpTracks[ii] << " which makes "
                              << hitMapVXD[mcpTracks[ii]] + hitMapSIT[mcpTracks[ii]] + hitMapTPC[mcpTracks[ii]]
                              << " hits, found " << " and " << hitMapVXD[mcpTracks[ii]] << " VXD hits " << std::endl;
      }
      if (foundFlag == 0) {
        streamlog_out(DEBUG4) << " Particle " << mcpTracks[ii] << " which makes "
                              << hitMapVXD[mcpTracks[ii]] + hitMapSIT[mcpTracks[ii]] + hitMapTPC[mcpTracks[ii]]
                              << " hits " << " and " << hitMapVXD[mcpTracks[ii]] << " VXD hits and "
                              << hitMapSIT[mcpTracks[ii]] << " SIT hits,  NOT found " << std::endl;
      }
