...
Residuals::Residuals() : Processor("Residuals") {
  ...
  registerInputCollection(LCIO::TRACK, "MarlinTrkTrack", "Input track collection", _trks,
                          std::string("MarlinTrkTracks"));

  registerProcessorParameter("TrackSystemName", "Name of the track fitting system to be used (DDKalTest, aidaTT, ... )",
                             _trkSystemName, std::string("DDKalTest"));

  registerProcessorParameter("SortWrtRadius", "Sort the hits wrt to their radius", _sortRad, bool(true));
}
...
void Residuals::processRunHeader(LCRunHeader* run) {
  ...
  streamlog_out(DEBUG4) << " processRun " << run->getRunNumber() << std::endl;
  _nRun++;
}
...
void Residuals::processEvent(LCEvent* evt) {
  ...
  for (int ii = 0; ii < MarlinTracks; ii++) {
    ...
    for (std::vector<std::pair<float, EVENT::TrackerHit*>>::iterator it = r_values.begin(); it != r_values.end();
         ++it) {
    ...
    if (fit_status == 0) {
      ...
      for (std::vector<TrackerHit*>::iterator lhit = NewTrkHits.begin(), endIter = NewTrkHits.end(); lhit < endIter;
           ++lhit) {
        ...
        streamlog_out(DEBUG4) << " subdetector " << idDecoder[lcio::LCTrackerCellID::subdet()] << " layer "
                              << idDecoder[lcio::LCTrackerCellID::layer()] << " Z position "
                              << (*lhit)->getPosition()[2] << std::endl;
        ...
        if (surf3 != NULL) {
          ...
          aidaTT::Vector2D localPos2 = surf3->globalToLocal(globalPos2);
          ...
          const aidaTT::Vector5 hp(ts_at_detel.getOmega() / dd4hep::mm, ts_at_detel.getTanLambda(),
                                   ts_at_detel.getPhi(), ts_at_detel.getD0() * dd4hep::mm,
                                   ts_at_detel.getZ0() * dd4hep::mm);
          aidaTT::Vector3D xx;
          ...
          if (doesIt2) {
            ...
            streamlog_out(DEBUG4) << " Global coordinates: track x point " << xx[0] << ", " << xx[1] << ", " << xx[2]
                                  << "hit position " << globpos2[0] << ", " << globpos2[1] << ", " << globpos2[2]
                                  << std::endl;

            aidaTT::Vector2D localUV2 = surf3->globalToLocal(xx);
...
void Residuals::check(LCEvent* evt) {
  streamlog_out(DEBUG4) << " Residuals::check event " << evt->getEventNumber() << std::endl;
