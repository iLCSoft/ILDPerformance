...
Skim::Skim() : Processor("Skim") {
  ...
  registerInputCollection(LCIO::MCPARTICLE, "MCParticleCollectionName", "Name of the MCParticle input collection",
                          _mcpColName, std::string("MCParticle"));

  registerOutputCollection(LCIO::MCPARTICLE, "MCParticleSkimmedCollectionName",
                           "Name of the skimmed MCParticle  output collection", _mcpSkimColName,
                           std::string("MCParticleSkimmed"));

  registerProcessorParameter("ThetaMin", "Minimum polar angle (in deg) to accept MC in skim", _thetaMin, double(40.));

  registerProcessorParameter("ThetaMax", "Maximum polar angle (in deg) to accept MC in skim", _thetaMax, double(140.));
}
...
void Skim::processEvent(LCEvent* evt) {
  ...
  streamlog_out(DEBUG4) << " processEvent " << evt->getEventNumber() << "  - " << evt->getRunNumber() << std::endl;
    ...
    for (int i = 0; i < mcpN; i++) {
      ...
      if (is_stable && is_charged && is_barrel) {
        if (mcp->ext<MCPKeep>() == true)
          continue; // particle already in skim
...
void Skim::check(LCEvent* evt) { streamlog_out(DEBUG4) << " Skim::check event " << evt->getEventNumber() << std::endl; }

void Skim::end() { streamlog_out(DEBUG4) << " processed " << _nEvt << " events in " << _nRun << " runs " << std::endl; }
...
void Skim::keepMCParticle(MCParticle* mcp, bool save_parents) {
  ...
  if (save_parents) {
    ...
    for (MCParticleVec::const_iterator pIt = parents.begin(); pIt != parents.end(); ++pIt) {
