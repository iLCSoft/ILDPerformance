...
void PResolutionL5(const char* modelName = "ILD_l5_v02") {
  ...
  TF1* fpr10 =
      new TF1("fpr10", "sqrt( 4.e-10 + 1.e-6/(x*x*0.17364817766*0.17364817766) )", 0.5, 200.0); // for theta =10 degree
  fpr10->SetLineColor(6);
  // fpr10->Draw("same");

  TF1* fpr20 =
      new TF1("fpr20", "sqrt( 4.e-10 + 1.e-6/(x*x*0.34202014332*0.34202014332) )", 0.5, 200.0); // for theta =20 degree
  fpr20->SetLineColor(4);
  // fpr20->Draw("same");

  TF1* fpr40 = new TF1("fpr40", "sqrt( 4.e-10 + 1.e-6/(x*0.64278760968*x*0.64278760968*0.64278760968*0.64278760968) )",
                       0.5, 200.0); // for theta =40 degree
  fpr40->SetLineColor(2);
  // fpr40->Draw("same");

  TF1* fpr85 = new TF1("fpr85", "sqrt( 4.e-10 + 1.e-6/(x*0.99619469809*x*0.99619469809*0.99619469809*0.99619469809) )",
                       0.5, 200.0); // for theta =85 degree
