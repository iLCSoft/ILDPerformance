...
void D0Resolution_comp() {
  ...
  for (int ii = 0; ii < SIZE_PA; ii++) {
    for (int i = 0; i < SIZE_M; i++) {
      ...
      TFile* f = new TFile(
          Form("/afs/desy.de/group/flc/pool/voutsina/SingleMuonTracks/DBD/Analysis/analysis_MuonsAngle_%d_Mom_%d.root",
               PA[ii], Mom[i]),
          "read");
      ...
      if ((Momentum[i] / PA[ii]) > 1 || (Mom[i] == 15 && PA[ii] == 10) || (Mom[i] == 10 && PA[ii] == 20)) {
        LimAxis = 0.04;
  ...
  for (int ii = 0; ii < SIZE_PA; ii++) {
    for (int i = 0; i < SIZE_M; i++) {
      ...
      printf(" I am studying file "
             "/nfs/dust/ilc/user/voutsina/testarea/CodeTest/fitting/ILDPerformance/trunk/tracking/Results/Analysis/"
             "analysis_MuonsAngle_%d_Mom_%d.root ",
             PA[ii], Mom[i]);
      ...
      // Loading the branches in EvalTree
      EvalTreeNew->SetBranchAddress("recoD0", &recoD0);
      EvalTreeNew->SetBranchAddress("trueD0", &trueD0);
  ...
  for (int kk = 0; kk < SIZE_M; kk++) {
    ...
    std::cout << " polar angle = 20, momentum " << Momentum[kk] << " kk " << kk << " sigma20 " << sigma20[kk]
              << " error " << error20[kk] << std::endl;
  }
  ...
  for (int kk = 0; kk < SIZE_M; kk++) {
    ...
    // std::cout << " polar angle = 10, momentum " << Momentum[kk] << " kk " << kk << " sigma10 " << sigma10[kk] << "
    // error " <<  error10[kk] << std::endl ;
  }
  ...
  for (int kk = 0; kk < SIZE_M; kk++) {
    ...
    // std::cout << " polar angle = 40, momentum " << Momentum[kk] << " kk " << kk << " sigma40 " << sigma40[kk] << "
    // error " <<  error40[kk] << std::endl ;
  }
  ...
  for (int kk = 0; kk < SIZE_M; kk++) {
    ...
    // std::cout << " polar angle = 85, momentum " << Momentum[kk] << " kk " << kk << " sigma85 " << sigma85[kk] << "
    // error " <<  error85[kk] << std::endl ;
  }
  ...
  for (int kk = 0; kk < SIZE_M; kk++) {
    ...
    std::cout << " polar angle = 20, momentum " << Momentum[kk] << " kk " << kk << " sigma_new20 " << sigma_new20[kk]
              << " error " << error_new20[kk] << std::endl;
  }
  ...
  for (int kk = 0; kk < SIZE_M; kk++) {
    ...
    // std::cout << " polar angle = 10, momentum " << Momentum[kk] << " kk " << kk << " sigma_new10 " << sigma_new10[kk]
    // << " error " <<  error_new10[kk] << std::endl ;
  }
  ...
  for (int kk = 0; kk < SIZE_M; kk++) {
    ...
    // std::cout << " polar angle = 40, momentum " << Momentum[kk] << " kk " << kk << " sigma_new40 " << sigma_new40[kk]
    // << " error " <<  error_new40[kk] << std::endl ;
  }
  ...
  for (int kk = 0; kk < SIZE_M; kk++) {
    ...
    // std::cout << " polar angle = 85, momentum " << Momentum[kk] << " kk " << kk << " sigma_new85 " << sigma_new85[kk]
    // << " error " <<  error_new85[kk] << std::endl ;
  }

  TCanvas* c_two = new TCanvas("IPResolutionBar", "IP Resolution", 200, 10, 700, 500);
  // c_two->Divide(1,2);
  // c_two->cd(1);
  ...
  Muon_plot20->SetTitle("IP Resolution");
  ...
  Muon_plot40->SetTitle("IP Resolution");
  ...
  Muon_plot10->SetTitle("IP Resolution");
  ...
  Muon_plot85->SetTitle("IP Resolution");
  Muon_plot85->SetMarkerColor(1);
  Muon_plot85->SetMarkerStyle(23);
  Muon_plot85->SetMarkerSize(1);
  // Muon_plot85 -> GetXaxis() -> SetTitle("P (GeV)");
  // Muon_plot85 -> GetYaxis() -> SetTitle("#sigma_{1/p_{T}}(GeV^{-1})");
  // Muon_plot85 -> SetMinimum( pow(10,-5) );
  //  Muon_plot85 -> SetMaximum( 2*pow(10, -1) );
  Muon_plot85->Draw("P");

  TGraphErrors* Muon_plot_new20 = new TGraphErrors(SIZE_M, Momentum, sigma_new20, zeros, error_new20);

  Muon_plot_new20->SetTitle("IP Resolution");
  ...
  Muon_plot_new40->SetTitle("IP Resolution");
  ...
  Muon_plot_new10->SetTitle("IP Resolution");
  ...
  Muon_plot_new85->SetTitle("IP Resolution");
