...
PIDTree::PIDTree() : Processor("PIDTree") {
  ...
  registerInputCollection(LCIO::LCRELATION, "MCTruth2RecoLinkCollectionName", "true - reco relation collection",
                          _trueToReco, std::string("MCTruthRecoLink"));

  registerInputCollection(LCIO::LCRELATION, "Reco2MCTruthLinkCollectionName", "reco - true relation collection",
                          _recoToTrue, std::string("RecoMCTruthLink"));

  registerInputCollection(LCIO::LCRELATION, "MCTruth2TrackLinkCollectionName", "true - track relation collection",
                          _trueToTrack, std::string("MCTruthMarlinTrkTracksLink"));
  ...
  registerInputCollection(LCIO::RECONSTRUCTEDPARTICLE, "BCALParticles", "Name of the BCALParticles input collection",
                          _BCALParticleCollectionName, std::string("BCALParticles"));

  registerInputCollection(LCIO::CLUSTER, "BCALCLusters", "Name of the BCAL clusters input collection",
                          _BCALClusterCollectionName, std::string("BCALCLusters"));

  registerInputCollection(LCIO::RECONSTRUCTEDPARTICLE, "PFOs", "particle flow objects", _pandoraPFOs,
...
void PIDTree::processEvent(LCEvent* evt) {
  ...
  streamlog_out(DEBUG) << " got mc2recoNav from " << mc2recoNav.getFromType() << " to " << mc2recoNav.getToType()
                       << std::endl;
  LCRelationNavigator reco2mcNav(evt->getCollection(_recoToTrue));
  streamlog_out(DEBUG) << " got reco2mcNav from " << reco2mcNav.getFromType() << " to " << reco2mcNav.getToType()
                       << std::endl;
  LCRelationNavigator mc2trackNav(evt->getCollection(_trueToTrack));
  streamlog_out(DEBUG) << " got mc2trackNav from " << mc2trackNav.getFromType() << " to " << mc2trackNav.getToType()
                       << std::endl;
  ...
  while (MCParticle* mcp = mcpIt.next()) {
    ...
    streamlog_out(DEBUG3) << " mcparticle " << imcp << " has PDG = " << mcp->getPDG()
                          << " and genstat = " << mcp->getGeneratorStatus() << std::endl;
    ...
    if (keep) {
      ...
      for (unsigned int irel = 0; irel < recovec.size(); irel++) {
        ...
        streamlog_out(DEBUG3) << " irel " << irel << ", recoweight = " << int(recoweightvec.at(irel))
                              << ", recoweight%10000 = " << trckweight << ", recoweight/10000 = " << caloweight
                              << std::endl;
      ...
      streamlog_out(DEBUG3) << " found reco particle for mcp at imaxtrackweight = " << imaxtrckweight
                            << " with weight = " << maxtrckweight << std::endl;
      streamlog_out(DEBUG3) << " found reco particle for mcp at imaxcaloweight = " << imaxcaloweight
      ...
      if (imaxtrckweight > -1 && imaxtrckweight < (int)recovec.size()) {
        ...
        for (unsigned imcptrck = 0; imcptrck < mcptrckvec.size(); imcptrck++) {
          if (mcptrckvec.at(imcptrck) == mcp) {
            ...
            streamlog_out(DEBUG3) << " found original MCP in mcptrckvec at position " << imcptrck << " with weight "
                                  << mcptrckweight << std::endl;
          }
        }
      } else {
      ...
      if (imaxcaloweight > -1 && imaxcaloweight < (int)recovec.size()) {
        ...
        for (unsigned imcpcalo = 0; imcpcalo < mcpcalovec.size(); imcpcalo++) {
          if (mcpcalovec.at(imcpcalo) == mcp) {
            ...
            streamlog_out(DEBUG3) << " found original MCP in mcpcalovec at position " << imcpcalo << " with weight "
                                  << mcpcaloweight << std::endl;
          }
        }
      } else {
      ...
      if (maxtrckweight > weightcut && mcptrckweight > weightcut) {
        ...
        streamlog_out(DEBUG3) << " found reco particle for mcp with bi-directional track link at imaxweight = "
                              << imaxweight << " with weight = " << maxweight << std::endl;
        ...
        streamlog_out(DEBUG3) << " found reco particle for mcp with bi-directional calo link at imaxweight = "
                              << imaxweight << " with weight = " << maxweight << std::endl;
      } else if (!recovec.empty()) { // don't warn about particles that are not reconstructed
        streamlog_out(WARNING) << " neither track nor cluster link have a weight larger than " << weightcut
                               << " in _both_ directions: " << std::endl;
        streamlog_out(WARNING) << " maxtrckweight =  " << maxtrckweight << ", mcptrckweight = " << mcptrckweight
                               << std::endl;
        streamlog_out(WARNING) << " maxcaloweight =  " << maxcaloweight << ", mcpcaloweight = " << mcpcaloweight
                               << std::endl;
      }
      ...
      if (maxweight > 0 && imaxweight > -1 && imaxweight < (int)recovec.size()) {
        ...
        streamlog_out(DEBUG3) << " trackvec has length = " << trackvec.size() << " for true PDG " << mcp->getPDG()
                              << std::endl;
        ...
        for (unsigned itrack = 0; itrack < trackvec.size(); itrack++) {
          ...
          if (fabs(trackweightvec.at(itrack) - maxtrckweight) < 0.001) {
            streamlog_out(DEBUG3) << " found track with weight " << trackweightvec.at(itrack) << ", filling dE/dx !"
                                  << std::endl;
            dedx = track->getdEdx();
            dedxerr = track->getdEdxError();
            weightdedx = maxtrckweight;
          } else {
            streamlog_out(DEBUG3) << " not taking track with weight " << trackweightvec.at(itrack)
                                  << " since difference with maxtrckweight = "
                                  << fabs(trackweightvec.at(itrack) - maxtrckweight) << std::endl;
          }
          if (trackvec.size() > 1) {
            streamlog_out(DEBUG3) << " ===> track " << itrack << " has innermost hit at "
                                  << track->getRadiusOfInnermostHit() << std::endl;
            if (mcp->getDaughters().size() > 0) {
              for (unsigned idaughter = 0; idaughter < mcp->getDaughters().size(); idaughter++) {
                streamlog_out(DEBUG3) << " ===> mcp daughter " << idaughter << " has PDG "
                                      << mcp->getDaughters()[idaughter]->getPDG() << std::endl;
              }
            }
          }
        }
        ...
        //
        //         // getParticleIDs ("S" !) gives back a VECTOR of ParticleID objects for all hypotheses, ordered by
        //         value ParticleIDVec likeliPID = pidh->getParticleIDs(rcp, pidh->getAlgorithmID("LikelihoodPID"));
        //
        ...
        //               if (hypcounter[jhyp] != 1) streamlog_out(ERROR) << " found wrong number of hypotheses:
        //               hypcounter [" << jhyp << "] = " << hypcounter[jhyp] << std::endl;
        //             }
        //           }
        //         }
        ...
        if (likeliPID && likeliPID->getParameters().size() > 4) {
          ...
          streamlog_out(DEBUG3) << " likeliPID has = " << likeliPID->getParameters().size() << " parameters "
                                << std::endl;
        }
        ...
        if (dedxPID && dedxPID->getParameters().size() > 17) {
          for (int ihyp = 0; ihyp < 5; ihyp++) {
            ...
            streamlog_out(DEBUG3) << pidh.getParameterNames(pidh.getAlgorithmID("dEdxPID"))[13 + ihyp] << " = "
                                  << distance[ihyp] << std::endl;
          }
      ...
      // IMPROVE HERE: CHECK if track or cluster exists!
        ...
        streamlog_out(DEBUG4) << "no ReconstructedParticle found for this mcp with PDG = " << mcp->getPDG() << " ! "
                              << std::endl;
