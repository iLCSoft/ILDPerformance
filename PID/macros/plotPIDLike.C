...
void plotPIDLike(const char* _filename) {
  ...
  hermTree->Draw("LiPDG_el>>h1",
                 "abs(trueCharge)>0.5&&isSeen>0.7&&trueP>1.0&&seenDEdx>0.&&seenDEdx<1.e-6&&abs(truePDG)==11");
  hermTree->Draw("LiPDG_el>>h2",
                 "abs(trueCharge)>0.5&&isSeen>0.7&&trueP>1.0&&seenDEdx>0.&&seenDEdx<1.e-6&&abs(truePDG)!=11");
  hermTree->Draw("LiPDG_mu>>h3",
                 "abs(trueCharge)>0.5&&isSeen>0.7&&trueP>1.0&&seenDEdx>0.&&seenDEdx<1.e-6&&abs(truePDG)==13");
  hermTree->Draw("LiPDG_mu>>h4",
                 "abs(trueCharge)>0.5&&isSeen>0.7&&trueP>1.0&&seenDEdx>0.&&seenDEdx<1.e-6&&abs(truePDG)!=13");
  hermTree->Draw("LiPDG_pi>>h5",
                 "abs(trueCharge)>0.5&&isSeen>0.7&&trueP>1.0&&seenDEdx>0.&&seenDEdx<1.e-6&&abs(truePDG)==211");
  hermTree->Draw("LiPDG_pi>>h6",
                 "abs(trueCharge)>0.5&&isSeen>0.7&&trueP>1.0&&seenDEdx>0.&&seenDEdx<1.e-6&&abs(truePDG)!=211");
  hermTree->Draw("LiPDG_ka>>h7",
                 "abs(trueCharge)>0.5&&isSeen>0.7&&trueP>1.0&&seenDEdx>0.&&seenDEdx<1.e-6&&abs(truePDG)==321");
  hermTree->Draw("LiPDG_ka>>h8",
                 "abs(trueCharge)>0.5&&isSeen>0.7&&trueP>1.0&&seenDEdx>0.&&seenDEdx<1.e-6&&abs(truePDG)!=321");
  hermTree->Draw("LiPDG_pr>>h9",
                 "abs(trueCharge)>0.5&&isSeen>0.7&&trueP>1.0&&seenDEdx>0.&&seenDEdx<1.e-6&&abs(truePDG)==2212");
  hermTree->Draw("LiPDG_pr>>h10",
                 "abs(trueCharge)>0.5&&isSeen>0.7&&trueP>1.0&&seenDEdx>0.&&seenDEdx<1.e-6&&abs(truePDG)!=2212");
