...
/// plot the PID efficiency and fake rate for a given particle type for various PID algorithms as function of momentum
/// and polar angle
...
void make_plots(const char* filename, const char* pidName, int pdgCode, const TString& pdfFile) {
  ...
  TH1F* hist_th_tnot =
      new TH1F("hist_th_tnot", "Cos theta of true particles which are not of the type in question", 11, -1., 1.);
  ...
  for (int ievt = 0; ievt < nevts; ++ievt) {
    ...
    for (int ipart = 0; ipart < npart; ipart++) {
      ...
      if (isSeen->at(ipart) < 0.5)
        continue; // require MC-truth link weight to be > 0.5

      if (seenCharge->at(ipart) == 0)
        continue; // only look at charged
      ...
      if (std::abs(thePDG->at(ipart)) != pdgCode)
        continue;
