  ...
  for (int i = 0; i < 5; ++i) { // loop over 5 PDG types
    ...
    for (int ievt = 0; ievt < nevts; ++ievt) {
      ...
      for (int ipart = 0; ipart < npart; ipart++) {
        ...
        if (trueP->at(ipart) < 1.0)
          continue; // momentum > 1GeV

        if (seenDEdx->at(ipart) < 0)
          continue;

        if (seenDEdx->at(ipart) > 1.e-6)
          continue;
    ...
    for (int iBins = 0; iBins < nBins; iBins++) {
      ...
      std::cout << particleName[i] << " ibin : " << iBins << " eff: " << eff[iBins] << " - pur: " << pur[iBins]
                << std::endl;
    }
